...
$.fn.html5uploader = function(opts){
	...
	this.each(function(){
		var _this = $(this);
		//先添加上file按钮和上传列表
		var inputstr = '<input class="uploadfile" style="visibility:hidden;" type="file" accept="image/jpeg" name="fileselect[]"';
		if(option.multi){
			inputstr += ' multiple="multiple"';
			}
		inputstr += '/>';
		...
		//创建文件对象
			  ...
			  onSelect: option.onSelect||function(files){
				 ...
				 //如果配置非自动上传，绑定上传事件
				 if(!option.auto){
					_this.find('.uploadbtn').off('click').on('click',function(){
