    ...
    $.fn.extend({
        jqfaceedit : function(options) {
            var defaults = {
                ...
                containerObj : '', //表情框父对象
                textareaid: 'inputMsg',//textarea元素的id
            ...
            return this.each(function() {
                ...
                $(Obj).bind("click", function(e) {
                    ...
                    for( i = 0; i < options.emotions.length; i++) {
                        faceHtml += '<li text=' + options.emotions[i].phrase + ' type=' + i + '><img title=' + options.emotions[i].phrase + ' src="../scripts/plugin/emotions/emotions/'+ options.emotions[i].url + '"  style="cursor:pointer; position:relative;"   /></li>';
                    }
                    faceHtml += '</ul></div>';
                    faceHtml += '</div></div>';
                    ...
                    container.find("#face").css("left","0").show(); 
                });
            });
        },
        //表情文字符号转换为html格式
                ...
                if(rs) {
                    for( i = 0; i < rs.length; i++) {
                        for( n = 0; n < em.length; n++) {
                            if(em[n].phrase == rs[i]) {
                                var t = "<img src='../scripts/plugin/emotions/emotions/"  + em[n].url + "' />";
