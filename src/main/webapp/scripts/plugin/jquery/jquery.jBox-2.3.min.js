...
(function ($) {
    $.jBox = function (content, options) {
        ...
        boxHtml.push('<div class="jbox-help-button jbox-button-panel" style="height:25px;padding:5px 0 1px 0;display:none;"></div>');
        ...
        $.each(content, function (stateName, stateOptions) {
            ...
            states.push('<div class="jbox-button-panel" style="height:30px;padding:4px 0 5px 0;text-align: right;' + (withButtons ? '' : 'display:none;') + '">');
            if (!options.isTip) {
            ...
            $.each(stateOptions.buttons, function (k, v) {
            	if(v || v == 'true'){
            		states.push('<button class="jbox-button-hover1" value="' + v + '" style="' + ieButtonFix + '">' + k + '</button>');
            	}else{
        ...
        var setFade = function () {
            if (options.showFade) {
                $.jBox.FadeBoxCount++;
                $($.support.msie ? 'html' : 'body').attr('style', 'overflow:hidden;padding-right:0px;');
...
_jBoxConfig.defaults = {
    ...
    title: 'jBox', /* 窗口的标题 */
    width: 200, /* 窗口的宽度，值为'auto'或表示像素的整数 */
...
_jBoxConfig.messagerDefaults = {
    ...
    width: 200, /* 信息的高度，值为'auto'或表示像素的整数 */
