...
function initSchedulePage(){
	$(":hidden[name=participant]").val('');
	$(":hidden[name=participantName]").val('');
	$(":hidden[name=contact]").val('');
	$(":hidden[name=contactName]").val('');
	$(":hidden[name=title]").val('');
	$(":hidden[name=startdate]").val('');
	$(":hidden[name=enddate]").val('');
	$(":hidden[name=desc]").val('');
	$(":hidden[name=status]").val('');
	$(":hidden[name=driority]").val('');
	$(":hidden[name=participant]").val('');
	$(":hidden[name=cycliValue]").val('');
	$(":hidden[name=cycliKey]").val('');
	$(":hidden[name=parentId]").val('');
	$(":hidden[name=parentType]").val('');
	$(":hidden[name=parentTypeName]").val('');
	$(":hidden[name=parentName]").val('');
	
	$("input[name=txtMsg]").val('');
	$("input[name=dateFlag]").val('');
	
	$("#contactMore").addClass("modal");
	$("#contactMore").find(":hidden[name=fstChar]").val('');
	$("#contactMore").find(".contactList").empty();
	$("#particMore").addClass("modal");
	$("#particMore").find(":hidden[name=fstChar]").val('');
	$("#particMore").find(".particList").empty();
	$("#particMore").find(".chartListSpan").empty();
	
	$(".relationList").css("display","none");
	$(".relationResp").css("display","none");
	$(".title").css("display","none");
	$(".titleResp").css("display","none");
	$(".beginTime").css("display","none");
	$(".beginTimeResp").css("display","none");
	$(".endTime").css("display","none");
	$(".endTimeResp").css("display","none");
	$(".status").css("display","none");
	$(".statusResp").css("display","none");
	$(".driority").css("display","none");
	$(".driorityResp").css("display","none");
	$(".partic").css("display","none");
	$(".particResp").css("display","none");
	$(".contact").css("display","none");
	$(".contactResp").css("display","none");
	$(".total").css("display","none");
	$(".submitDiv").css("display","none");
	$(".txtDiv").val('').css("display","none");
	$(".cycli").css("display","none");
	$(".cycliResp").css("display","none");
	$(".cycliResp").find(".showTxt").html('');
}
...
function initRelation(){
	...
	p.reTypeHrefs.click(function(){
		...
		if(k === "Campaigns"){
			p.reTypeSeachUrl.val('<%=path%>/campaigns/asyList');//项目查询URL
...
function compileReListData(data){
    ...
    var v = '';
	$(data).each(function(){
		v += '<span style="height:25px;line-height:25px;">';
...
function compileParticData(d){
	...
	p.particBtn.click(function(){
		...
		if($(":hidden[name=participant]").val()==''){
			$(".myMsgBox").css("display","").html("请选择内部参与人!");
	    	$(".myMsgBox").delay(2000).fadeOut();
    	   return;
		}
		p.particResp.css("display",'');
...
function compOtherContact(d){
	...
	p.conChoBtn.click(function(){
		if(!(p.conChoList.find("a").hasClass("checked"))){
...
function initCycli(){
	...
	p.cycliContent.find("a").unbind("click").bind("click", function(event){
		var k = $(this).attr("key"), n = $(this).html();
		$(":hidden[name=cycliKey]").val(k);
		p.cycliValue.val(n);
		p.cycliRespStxt.html(n);
		//显示
		$(".cycliResp").css("display","");
		p.total.css("display",'');
		//提交
...
function totalMsg(){
	var tmp = ['<h1 style="font-size: 15px;">您创建的任务汇总如下所示:</h1><br>',
				 ...
				 '【7】.  内部参与人: <span style="color:blue">'+ p.particRespStxt.html() +'</span><br>',
				 '【8】.  联系人: <span style="color:blue">'+ p.contactName.val() +'</span><br>',
				 '【9】.  周期: <span style="color:blue">'+ p.cycliValue.val() +'</span><br><br>',
...
<body>
	...
	<div id="taskCreate">
		...
		<div class="site-card-view ">
			<!-- 提交用户注册数据的表单 -->
			<form name="taskForm" action="<%=path%>/schedule/save" method="post">
							...
							<div class="cloudContent links">
								...
								<div style="word-wrap: break-word; font-family: 'Microsoft YaHei';">
									...
									<div style="margin-top: 10px;">
										...
										<a href="javascript:void(0)" key="Campaigns">市场活动 </a>&nbsp;&nbsp;
									</div>
								</div>
							</div>
						</div>
		...
		<!-- 任务优先级 -->
		<div class="chatItem you driority" style="background: #FFF;display:none;">
			<div class="chatItemContent">
				<img class="avatar" src="<%=path%>/scripts/plugin/wb/css/images/dc.png" >
				<div class="cloud cloudText" style="margin: 0 0 0 10px;">
					<div class="cloudPannel">
		...
		<!-- 任务优先级    回复-->
		<div class="chatItem me driorityResp" style="background: #FFF;display:none;">
			<div class="chatItemContent">
				<img class="avatar" width="40px" height="40px"
					src="${headimgurl}">
				<div class="cloud cloudText" style="margin: 0 15px 0 0;">
		...
		<div>
			...
			<div class="submitDiv"  style="display:none;margin-top:5px;text-align:center;">
				...
				<div class="button-ctrl schesubmitinclude">
					<fieldset class="">
						<a href="javascript:void(0)" onclick="confirmSchedule()" class="btn btn-block " 
	...
	<div id="contactMore" class=" modal">
		...
		<div class="page-patch">
			...
			<div class=" flooter" style="font-size: 14px;background: #F7F7F7;border-top: 1px solid #ADA7A7;padding-top: 4px;">
				<input class="btn btn-block contactBtn" type="button" value="确&nbsp;定" style="width: 95%;margin: 3px 0px 3px 8px;" >
			</div>
		</div>
	</div>
	...
	<div id="particMore" class=" modal">
		...
		<div class="page-patch">
			...
			<div class=" flooter" style="font-size: 14px;background: #F7F7F7;border-top: 1px solid #ADA7A7;padding-top: 4px;">
				<input class="btn btn-block particBtn" type="button" value="确&nbsp;定" style="width: 95%;margin: 3px 0px 3px 8px;" >
			</div>
		</div>
	</div>
