...
// 	                    }
	                ...
	                series: {
	    	            ...
	    	            point: {
	    	                events: {
	    	                    click: function(e) {
	    	                    	debugger;
	    	                    	var str = (getDate(e.point.category)).split(",");
		 							var month_url = "<a href='"+encodeURI("../quota/month?source=url&publicId=${publicId}&openId=${openId}&startDate="+str[0]+"&endDate="+str[1]+"&assignerId=${assignerId}&addAssigner=${addAssigner}") +"' target='_parent'>月份分析报表</a>";
		 							$("#analytics_month").html(month_url);
		 							var detail_url = "<a href='"+encodeURI("../../contract/list?viewtype=analyticsview&publicId=${publicId}&openId=${openId}&startdate="+str[0]+"&endDate="+str[1]+"&assignerId=${assignerId}&addAssigner=${addAssigner}") +"' target='_parent'>合同明细表</a>";
    ...
    //通过季度得到开始时间和结束时间
    function getDate(quarter){
    	var year = quarter.split("-")[0];
    	var qua = quarter.split("-")[1];
    	var startdate='';
    	var enddate='';
    	if("Q1"==qua){
    		startdate=year+"-01";
...
<!-- 			<i class="icon-menu"><b></b></i> -->
	...
	<div id="analytics_div" class="view site-recommend">
		<div class="recommend-box">
				<!-- <h4>详情</h4> -->
				<form name="gatheringForm" action="<%=path%>/analytics/quota/quarter?openId=${openId}&publicId=${publicId}" method="post" novalidate="true" >
						...
						<table style="border:1px solid #EEEEEE;">
							<tbody>
								...
								<tr style="border-bottom:1px solid #EEEEEE;">
									<c:if test = "${qa.quarter eq '合计'}">
										<td>${qa.quarter}</td>
									</c:if>
									<c:if test = "${qa.quarter ne '合计'}">
										<td><a href="<%=path%>/contract/list?viewtype=analyticsview&openId=${openId}&publicId=${publicId}&quarter=${qa.quarter}&assignerId=${assignerId}&addAssigner=${addAssigner}" target="_parent">${qa.quarter}</a></td>
									</c:if>
									<td>${qa.quotaamt}</td>
									<td>${qa.recamt}</td>
									<td>${qa.growthrate}</td>
	...
	<div id="gotodiv" style="position:absolute;top:40%;left:35%;z-index:999;background-color:#EEEEEE;padding:2px;display:none;">
		...
		<div id="analytics_detail" style="border-bottom:1px solid #EEEEEE;background-color:#FFF;padding:10px 20px 10px 20px;"></div>
	</div>
