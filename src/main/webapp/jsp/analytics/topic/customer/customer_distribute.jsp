...
$(function(){
	...
	//如果同有数据，不加载报表
	if(recordcount == "0"){
		$("#analytics_customer_distribute").html("没有找到数据");
		$("#analytics_customer_distribute").css("padding-top","80px");
		$("#analytics_customer_distribute").css("min-height","200px");
		return;
	}
	...
	if("no" == "${dataFlg}"){
		...
		if("<%=flag%>"!="hidden"){
			if(<%=request.getAttribute("dimession")%>!=null&&<%=request.getAttribute("dimession")%>!=''){
				loadhigh2(<%=request.getAttribute("dimession")%>);
			}else{
				$("#analytics_customer_distribute").html("没有找到数据");
				$("#analytics_customer_distribute").css("padding-top","80px");
				$("#analytics_customer_distribute").css("min-height","200px");
    ...
    function load2() {
    	if("<%=flag%>"=="hidden"){
			...
			//异步查询报表的数据
		 	 $.ajax({
					type : 'get',
					url : '<%=path%>/analytics/customer/ajaxdistribute',			
			        data: {
			        	viewtype: '<%=viewtype%>',
				    ...
				    success: function(data){
				    	if(data){
				    		...
				    		if(!d){
				    			$("#analytics_customer_distribute").html("没有找到分析数据");
				        		$("#analytics_customer_distribute").css("padding-top","80px");
				        		$("#analytics_customer_distribute").css("min-height","400px");
				    		}
				    		var dataObj = [];
	 ...
	 function loadhigh2(dimession){
		 	require(
                 ...
                 function (ec) { 
                     ...
                     $("#analytics_customer_distribute").css("min-height","400px");
                     var myChart = ec.init(document.getElementById('analytics_customer_distribute')); 
                     option = {
                     	    title : {
                     	        text: '客户地域分析',
                     	        subtext: '',
		...
		<div class="list-group-item-bd"
			...
			<p>
				<div class="form-control select _viewtype_select_">
					<div class="select-box2"><span class="viewtypelabel">地理分布分析</span>&nbsp;<img src="<%=path%>/image/dropdown.png" width="16px;"></div>
					<%--<div class="select-box viewtypelabel" style="color: #000">企业地理分布分析</div> --%>
				</div>
			</p>
		</div>
	</div>
	...
	<div id="analytics_div" class="view site-recommend">
		<div class="recommend-box">
			...
			<form name="expenseForm"
				action="<%=path%>/analytics/customer/industry"
				...
				<div class="site-card-view">
					<div class="card-info">
						<div id="analytics_customer_distribute"
