...
<script src="<%=path%>/scripts/plugin/echarts-2.1.10/build/dist/echarts.js"></script>
    ...
    $(function(){
    	...
    	//如果同有数据，不加载报表
    	if(recordcount == "0"){
    		$("#analytics_customer_contribution").html("没有找到数据");
    		$("#analytics_customer_contribution").css("padding-top","80px");
    		$("#analytics_customer_contribution").css("min-height","400px");
    		return;
    	} 
    	if("no" == "${dataFlg}"){
    		$("#analytics_customer_contribution").html("没有找到数据");
    		...
    		$("#analytics_customer_contribution").css("min-height","400px");
	   		...
	   		if("<%=flag%>"!="hidden"){
	   			...
	   			loadhigh1(<%=request.getAttribute("dimession")%>,<%=request.getAttribute("fact")%>);
	   			}else{
	   				$("#analytics_customer_contribution").html("没有找到数据");
	   	    		$("#analytics_customer_contribution").css("padding-top","80px");
	   	    		$("#analytics_customer_contribution").css("min-height","400px");
    ...
    function load() {
    		if("<%=flag%>"=="hidden"){
    			...
    			//异步查询报表的数据
    			  $.ajax({
    					type : 'get',
    					url : '<%=path%>/analytics/customer/ajaxcontribution',			
    			        data: {
    			        	viewtype: '<%=viewtype%>',
    				    ...
    				    success: function(data){
    				    	if(data){
    				    		...
    				    		if(!d){
    				    			$("#analytics_customer_contribution").html("没有找到分析数据");
    				        		$("#analytics_customer_contribution").css("padding-top","80px");
    				        		$("#analytics_customer_contribution").css("min-height","400px"); 
    				    		}else{
    ...
    function updatecontribution(){
       	var value = $("select[name=contributionSel]").val();
    	$(":hidden[name=contribution]").val(value);
    }
    function loadhigh1(fact,dimession){
    	require(
                ...
                function (ec) { 
                    ...
                    $("#analytics_customer_contribution").css("min-height","400px");
                    var myChart = ec.init(document.getElementById('analytics_customer_contribution')); 
                    option = {
                    	    title : {
                    	        text: '客户贡献度分析',
                    	        subtext: ''
		...
		<div class="list-group-item-bd" style="width: 180px; margin: 0 auto; padding-top: 5px;">
			<p>
				<div class="form-control select _viewtype_select_">
					<div class="select-box2"><span class="viewtypelabel">客户贡献度分析</span>&nbsp;<img src="<%=path%>/image/dropdown.png" width="16px;"></div>
					<%--<div class="select-box viewtypelabel" style="color:#000">客户贡献度分析</div> --%>
				</div>
			</p>
		</div>
	</div>
...
<!-- 下拉菜单选项 -->
	...
	function viewtypeClick_(){
		...
		<a
			...
			<div style="float: left; padding: 10px; width: 50%;">
				<img src="<%=path%>/image/icon_cirle.png">&nbsp;行业分析
			</div>
		</a> 
		<a
	...
	<div id="analytics_div" class="view site-recommend">
		<div class="recommend-box">
				<!-- <h4>详情</h4> -->
				<form name="customerForm" action="<%=path%>/analytics/customer/contribution" method="post" novalidate="true" >
		 <%if(!"hidden".equals(flag)){%>
				<div  class="search"  style="width:100%;border-bottom:1px solid #AAA;padding-left:2px;padding-right:2px;line-height:50px;">
					<span>贡献度类型：</span>
					 <span> <select name="contributionSel"
								onchange="updatecontribution()" style="height: 2.2em; width : 18%">
								<option value="">请选择类型</option>
								<option value="top">最多</option>
								<option value="last">最少</option>
							</select>
						<input name="contribution" id="contribution" value="" type="hidden"  >
					</span> 
			...
			<%} %>	
				<div class="site-card-view" >
					<div class="card-info" >
				<div id="analytics_customer_contribution" style="text-align: center; color: #AAAAAA"></div>		
					</div>
				</div>
				</form>
				<%if(!"hidden".equals(flag)){%>
