	                ...
	                pie: {
	                    ...
	                    events:{
	                        click: function(e) {
	 							...
	 							var subtype = e.point.name.split("-");
	 							var depart_url = "<a href='"+encodeURI("../expense/depart?source=url&type=${type}&subtype="+subtype[0]+"&subtypename="+subtype[1]+"&publicId=${publicId}&openId=${openId}&startDate="+startDate+"&endDate="+endDate+"&assignerId="+assignerId+"&addAssigner="+addAssigner+"&depart=${depart}") +"' target='_parent'>部门费用分析</a>";
	 							$("#analytics_depart").html(depart_url);
	 							
	 							var type_url = "<a href='"+encodeURI("../expense/type?source=url&type=${type}&subtype="+subtype[0]+"&subtypename="+subtype[1]+"&publicId=${publicId}&openId=${openId}&startDate="+startDate+"&endDate="+endDate+"&assignerId="+assignerId+"&addAssigner="+addAssigner+"&depart=${depart}") +"' target='_parent'>费用用途占比分析</a>";
	 							$("#analytics_type").html(type_url);
	 							
	 							var detail_url = "<a href='"+encodeURI("../../expense/list?viewtype=analyticsview&type=${type}&approval=approved&subtype="+subtype[0]+"&publicId=${publicId}&openId=${openId}&startDate="+startDate+"&endDate="+endDate+"&exAssigner="+assignerId+"&depart=${depart}") +"' target='_parent'>费用明细表</a>";
...
<body>
		...
		<div class="list-group-item-bd" style="width: 180px; margin: 0 auto; padding-top: 5px;">
			<p>
				<div class="form-control select _viewtype_select">
					<div class="select-box viewtypelabel" style="color:#000">费用类型占比分析</div>
				</div>
			</p>
		</div>
	</div>
	...
	<div id="gotodiv" style="position:absolute;top:40%;left:35%;z-index:999;background-color:#EEEEEE;padding:2px;display:none;">
		...
		<div id="analytics_depart" style="border-bottom:1px solid #EEEEEE;background-color:#FFF;padding:10px 20px 10px 20px;"></div>
