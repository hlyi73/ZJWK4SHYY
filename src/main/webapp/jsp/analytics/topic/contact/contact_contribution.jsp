...
<link rel="stylesheet" href="<%=path%>/css/page.css" id="style_color">
    ...
    $(function () {
    	loadhigh();
    	if("no" == "${dataFlg}"){
    		$("#analytics_contact_contribution").html("没有找到数据");
    		$("#analytics_contact_contribution").css("padding-top","80px");
    		$("#analytics_contact_contribution").css("min-height","400px");
    	}else{
    		
    		if("<%=flag%>"=="hidden"){
    			$("#_viewtype_menu__").css("display","none");
    			...
    			//异步查询报表的数据
    			  $.ajax({
    					type : 'get',
    					url : '<%=path%>/analytics/contact/ajaxcontribution',			
    			        data: {
    			        	openId:'${openId}',
    			        	publicId:'${publicId}',
    			        	viewtype: '<%=viewtype%>',
    				    ...
    				    success: function(data){
    				    	if(data){
    				    		...
    				    		if(!d){
    				    			$("#analytics_contact_contribution").html("没有找到分析数据");
    				        		$("#analytics_contact_contribution").css("padding-top","80px");
    				        		$("#analytics_contact_contribution").css("min-height","400px");
    				    		}
    ...
    function loadhigh(fact,dimession){
    	chart = new Highcharts.Chart( {
            chart: {
                type: 'bar',
                renderTo : 'analytics_contact_contribution'
            },
            title: {
                text: '客户贡献度分析',
                style: {
		...
		<div class="list-group-item-bd" style="width: 180px; margin: 0 auto; padding-top: 5px;">
			<p>
				<div class="form-control select _viewtype_select_">
					<div class="select-box viewtypelabel" style="color:#000">联系人贡献度分析</div>
				</div>
			</p>
		</div>
	</div>
...
<!-- 下拉菜单选项 -->
	...
	<div id="analytics_div" class="view site-recommend">
		<div class="recommend-box">
				<!-- <h4>详情</h4> -->
				<form name="customerForm" action="<%=path%>/analytics/customer/contribution?openId=${openId}&publicId=${publicId}" method="post" novalidate="true" >
		 <%if(!"hidden".equals(flag)){%>
			...
			<%} %>	
				...
				<div class="site-card-view">
					<div class="card-info">
						<div id="analytics_contact_contribution" style="text-align:center;color:#AAAAAA;font-family:Microsoft YaHei;"></div>						
					</div>
				</div>
				</form>
				<%if(!"hidden".equals(flag)){%>
