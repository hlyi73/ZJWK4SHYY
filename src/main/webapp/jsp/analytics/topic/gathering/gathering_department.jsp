...
<link rel="stylesheet" href="<%=path%>/css/page.css" id="style_color">
    ...
    $(function () {
    	if("no" == "${dataFlg}"){
	        ...
	        $('#analytics_gathering').highcharts({
	        	chart: {
	    	                ...
	    	                events: {
	    	                    click: function(e) {
	    	                    	...
	    	                    	var depart = e.point.category.split("-")[0];
		 							var month_url = "<a href='"+encodeURI("../gathering/month?source=url&publicId=${publicId}&openId=${openId}&month=${month}&startDate=${startDate}&endDate=${endDate}&assignerId=${assignerId}&addAssigner=${addAssigner}&depart="+depart) +"' target='_parent'>回款分析表(月份)</a>";
		 							$("#analytics_month").html(month_url);
		 							
		 							var customer_url = "<a href='"+encodeURI("../gathering/customer?source=url&publicId=${publicId}&openId=${openId}&month=${month}&startDate=${startDate}&endDate=${endDate}&assignerId=${assignerId}&addAssigner=${addAssigner}&depart="+depart) +"' target='_parent'>企业回款分析表</a>";
		 							$("#analytics_customer").html(customer_url);
		 							
		 							var detail_url = "<a href='"+encodeURI("../../gathering/list?viewtype=analyticsview&publicId=${publicId}&openId=${openId}&month=${month}&startDate=${startDate}&endDate=${endDate}&exAssigner=${assignerId}&depart="+depart) +"' target='_parent'>回款明细表</a>";
...
<div id="site-nav" class="navbar">
		...
		<div class="list-group-item-bd" style="width: 180px; margin: 0 auto; padding-top: 5px;">
			<p>
				<div class="form-control select _viewtype_select">
					<div class="select-box2"><span class="viewtypelabel">回款分析-by部门</span>&nbsp;<img src="<%=path%>/image/dropdown.png" width="16px;"></div>
					<%--<div class="select-box viewtypelabel" style="color:#000">回款分析 - by部门</div> --%>
				</div>
			</p>
		</div>
	</div>
	...
	<!-- 下拉菜单选项 -->
		...
		function viewtypeClick(){
			if($("#_viewtype_menu").css("display") == "none"){
				...
				$("#_viewtype_menu").animate({height : 160}, [ 10000 ]);
	...
	<div id="analytics_div" class="view site-recommend">
		<div class="recommend-box">
				<!-- <h4>详情</h4> -->
				<form name="gatheringForm" action="<%=path%>/analytics/gathering/depart?openId=${openId}&publicId=${publicId}" method="post" novalidate="true" >
	...
	<div id="gotodiv" style="position:absolute;top:40%;left:35%;z-index:999;background-color:#EEEEEE;padding:2px;display:none;">
		...
		<div style="clear:both;"></div>
		<div id="analytics_month" style="background-color:#FFF;padding:10px 20px 10px 20px;"></div>
		<div id="analytics_customer" style="border-bottom:1px solid #EEEEEE;background-color:#FFF;padding:10px 20px 10px 20px;"></div>
