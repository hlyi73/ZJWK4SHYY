	    	            ...
	    	            point: {
	    	                events: {
	    	                    click: function(e) {
	    	                    	...
	    	                    	var startDate = month;
		 							var endDate = month;
// 		 							var depart_url = "<a href='"+encodeURI("../gathering/depart?source=url&publicId=${publicId}&depart=${depart}&openId=${openId}&startDate="+startDate+"&endDate="+endDate+"&assignerId=${assignerId}&addAssigner=${addAssigner}&month="+month) +"' target='_parent'>部门回款分析表</a>";
// 		 							$("#analytics_depart").html(depart_url);
		 							
		 							var customer_url = "<a href='"+encodeURI("../gathering/customer?source=url&publicId=${publicId}&openId=${openId}&depart=${depart}&startDate="+startDate+"&endDate="+endDate+"&assignerId=${assignerId}&addAssigner=${addAssigner}&month="+month) +"' target='_parent'>企业回款分析表</a>";
		 							$("#analytics_customer").html(customer_url);
		 							
		 							var detail_url = "<a href='"+encodeURI("../../gathering/list?viewtype=analyticsview&publicId=${publicId}&openId=${openId}&depart=${depart}&startDate="+startDate+"&endDate="+endDate+"&assignerId=${assignerId}&addAssigner=${addAssigner}&month="+month) +"' target='_parent'>回款明细表</a>";
		...
		<div class="list-group-item-bd" style="width: 180px; margin: 0 auto; padding-top: 5px;">
			<p>
				<div class="form-control select _viewtype_select">
					<div class="select-box2"><span class="viewtypelabel">回款分析-by月</span>&nbsp;<img src="<%=path%>/image/dropdown.png" width="16px;"></div>
					<%--<div class="select-box viewtypelabel" style="color:#000">回款分析 - by月</div> --%>
				</div>
			</p>
		</div>
	</div>
...
<!-- 			</a> -->
	...
	<div id="analytics_div" class="view site-recommend">
		<div class="recommend-box">
				<!-- <h4>详情</h4> -->
				<form name="gatheringForm" action="<%=path%>/analytics/gathering/month?openId=${openId}&publicId=${publicId}" method="post" novalidate="true" >
