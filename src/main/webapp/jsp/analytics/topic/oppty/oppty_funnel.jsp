...
<head>
    ...
    <script src="<%=path%>/scripts/plugin/hcharts/funnel.js" type="text/javascript"></script>
	<!--框架样式-->
...
<script src="<%=path%>/scripts/plugin/wb/js/mobiscroll.core-2.5.2.js" type="text/javascript"></script>
    ...
    $(function () {
    	if("no" == "${dataFlg}"){
    		$("#analytics_oppty_funnel").html("没有找到数据");
    		$("#analytics_oppty_funnel").css("padding-top","80px");
    		$("#analytics_oppty_funnel").css("min-height","200px");
    	}else{
        $('#analytics_oppty_funnel').highcharts({
            chart: {
                type: 'funnel',
                marginRight: 100
            },
    	                ...
    	                events: {
    	                    click: function(e) {
    	                    	...
    	                    	var key = initSeriesData(name);
	 							var detail_url = "<a href='"+encodeURI("../../oppty/opptylist?viewtype=analyticsview&cstartDate=${startDate}&cendDate=${endDate}&assignerId=${assignerId}&salesstage="+key) +"' target='_parent'>业务机会明细表</a>";
	 							$("#analytics_detail").html(detail_url);
	 							$("#analytics_failure").css("display","none");
    	                    	if("丢单结束"==name||"放弃结束"==name){
    	                    		var failure_url = "<a href='"+encodeURI("../../analytics/oppty/failure?startDate=${startDate}&endDate=${endDate}&assignerId=${assignerId}&salestage="+key) +"' target='_parent'>业务机会失败分析表</a>";
		 							$("#analytics_failure").html(failure_url);
		 							$("#analytics_failure").css("display","");
    	                    	}
	 							$("#gotodiv").css("display","");
    	                    }
    	                }
    ...
    function initSeriesData(k){
    	if('${clist}'){
	    	...
	    	var value='';
    		$(data).each(function(){
    			var key = this.opptyStage;
    			var name = this.opptyStagename;
    			if(k==name){
    				value=key;
    				return;
    			}
...
<body>
	<div id="site-nav" class="navbar">
		...
		<h3 style="padding-right:45px;">销售漏斗</h3>
	</div>
	<jsp:include page="/common/navbar.jsp"></jsp:include>
	<div id="analytics_div" class="view site-recommend">
		<div class="recommend-box">
				<!-- <h4>详情</h4> -->
				<form name="opptyForm" action="<%=path%>/analytics/oppty/funnel" method="post" novalidate="true" >
				<div style="width:100%;border-bottom:1px solid #AAA;padding-left:2px;padding-right:2px;line-height:50px;">
					<span>时间</span>
					<input name="startDate" id="startDate" value="" style="width:80px;" type="text" placeholder="开始月份" readonly="">
					<span">-</span>
					<input name="endDate" id="endDate" value="" style="width:80px;" type="text" placeholder="结束月份" readonly="">	
				...
				<div class="site-card-view">
					<div class="card-info">
						<div id="analytics_oppty_funnel" style="text-align:center;color:#AAAAAA"></div>						
					</div>
				</div>
				</form>
