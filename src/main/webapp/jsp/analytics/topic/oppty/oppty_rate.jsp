...
<script type="text/javascript">
    $(function () {
    	...
    	if("no" == "${dataFlg}"){
    		$("#analytics_oppty_rate").html("没有找到数据");
    		$("#analytics_oppty_rate").css("padding-top","80px");
    		$("#analytics_oppty_rate").css("min-height","200px");
    	}else{
	        $('#analytics_oppty_rate').highcharts({
	            chart: {
				...
				plotOptions : {
					pie : {
						...
						events: {
							 click: function(e) {
		   	                    	debugger;
		   	                    	var province = initSeriesData(e.point.name);
		   	                    	if(province == ""){
		   	                    		province = "-999";
		   	                    	}
		   	                    	var detail_url = "<a href='"+encodeURI("../../oppty/opptylist?viewtype=analyticsview&publicId=${publicId}&openId=${openId}&startDate=${startDate}&endDate=${endDate}&assignerId=${assignerId}&rate="+rate) +"' target='_parent'>业务机会明细表</a>";
...
<body>
		...
		<div class="list-group-item-bd" style="width: 180px; margin: 0 auto; padding-top: 5px;">
			<p>
				<div class="form-control select _viewtype_select">
					<div class="select-box viewtypelabel" style="color:#000">商机成单率分析</div>
				</div>
			</p>
		</div>
	</div>
	...
	<div id="analytics_div" class="view site-recommend">
		<div class="recommend-box">
			<!-- <h4>详情</h4> -->
			<form name="rateForm"
				action="<%=path%>/analytics/oppty/rate?openId=${openId}&publicId=${publicId}" method="post"
				novalidate="true">
				<div
