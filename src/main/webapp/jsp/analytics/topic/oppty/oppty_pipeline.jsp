...
<script src="<%=path%>/scripts/plugin/echarts-2.1.10/build/dist/echarts.js"></script>
    ...
    $(function () {
    	...
    	//如果同有数据，不加载报表
    	if(recordcount == "0"){
    		$("#analytics_oppty_pipeline").html("没有找到数据");
    		$("#analytics_oppty_pipeline").css("padding-top","80px");
    		$("#analytics_oppty_pipeline").css("min-height","200px");
    		return;
    	}
    ...
    function load1(){
    	if("<%=flag%>"=="hidden"){
			...
			//异步查询报表的数据
			  $.ajax({
					type : 'get',
					url : '<%=path%>/analytics/oppty/ajaxpipeline',			
			        data: {
			        	viewtype: '<%=viewtype%>',
			        	opptyname: '<%=opptyname%>',
			        	salestage: '<%=salestage%>',
				    ...
				    success: function(data){
				    	if(data){
    				    	...
    				    	$(d).each(function(n,item){
    				    		x.push(this.opptyStagename);
    				    		if(this.opptyAmount != ''){
    				    			y.push(parseFloat(this.opptyAmount));
    				    		}else{
    				    			y.push(0);
    				    		}
    				    	});
    ...
    function loadhigh1(fact,dimession){
 	   ...
 	   require(
          ...
          $("#analytics_oppty_pipeline").css("min-height","400px");
         var myChart = ec.init(document.getElementById('analytics_oppty_pipeline')); 
 	  	option = {
       	    title : {
       	        text: '销售管道分析 ',
       	        subtext: ''
...
<%if(!"hidden".equals(flag)){%>
		...
		<div class="list-group-item-bd" style="width: 180px; margin: 0 auto; padding-top: 5px;">
			<p>
				<div class="form-control select _viewtype_select_">
					<div class="select-box2"><span class="viewtypelabel">销售管道分析</span>&nbsp;<img src="<%=path%>/image/dropdown.png" width="16px;"></div>
				</div>
			</p>
		</div>
	</div>
	...
	function viewtypeClick_(){
		if($("#_viewtype_menu_").css("display") == "none"){
			...
			$("#_viewtype_menu_").animate({height : 200}, [ 10000 ]);
	...
	<div id="analytics_div" class="view site-recommend">
		<div class="recommend-box">
				<!-- <h4>详情</h4> -->
				<form name="opptyForm" action="<%=path%>/analytics/oppty/pipeline?openId=${openId}&publicId=${publicId}" method="post" novalidate="true" >
		<%if(!"hidden".equals(flag)){%>
		...
		<%} %>
				...
				<div class="site-card-view">
					<div class="card-info">
						<div id="analytics_oppty_pipeline" style="text-align:center;color:#AAAAAA;font-family:Microsoft YaHei;"></div>						
					</div>
				</div>
				</form>
					<%if(!"hidden".equals(flag)){%>
