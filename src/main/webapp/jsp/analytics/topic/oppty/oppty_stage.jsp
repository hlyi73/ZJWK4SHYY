...
<head>
    ...
    $(function () {
    	...
    	//如果同有数据，不加载报表
    	if(recordcount == "0"){
    		$("#analytics_oppty_stage").html("没有找到数据");
    		$("#analytics_oppty_stage").css("padding-top","80px");
    		$("#analytics_oppty_stage").css("min-height","200px");
    		return;
    	}    	
    	if("no" == "${dataFlg}"){
    		$("#analytics_oppty_stage").html("没有找到数据");
    		...
    		if("<%=flag%>" != "hidden"){
    			var data='<%=request.getAttribute("fact")%>';
    			var y = [];
    		 	var x = [];
    			if(data){
    			...
    			loadhigh2(x,y,<%=request.getAttribute("dimession")%>); 
    		}
    	}
        initDatePicker();
    	initDate();
    ...
    function load2(){
    	if("<%=flag%>" == "hidden"){
			...
			$.ajax({
				type: "get",
				url: "<%=path%>/analytics/oppty/ajaxstage",
				data: {
		        	viewtype: '<%=viewtype%>',
		        	opptyname: '<%=opptyname%>',
		        	salestage: '<%=salestage%>',
	...
	function loadhigh2(x,y,dimession){
		try{
		   require(
                   [
                       'echarts',
             ...
             $("#analytics_oppty_stage").css("min-height","400px");
            var myChart = ec.init(document.getElementById('analytics_oppty_stage')); 
            option = {
            	    tooltip : {
            	        trigger: 'axis'
            	    },
		...
		<div class="list-group-item-bd" style="width: 180px; margin: 0 auto; padding-top: 5px;">
			<p>
				<div class="form-control select _viewtype_select_">
					<div class="select-box2"><span class="viewtypelabel">业务机会统计分析-by月</span>&nbsp;<img src="<%=path%>/image/dropdown.png" width="16px;"></div>
				</div>
			</p>
		</div>
	</div>
...
<div class="_viewtype_menu_class" id="_viewtype_menu_" style="width:100%;padding:10px;background-color:#fff;display:none;text-align:left;font-size:14px;"> 
	...
	<div id="analytics_div" class="view site-recommend">
		<div class="recommend-box">
				<!-- <h4>详情</h4> -->
				<form name="opptyForm" action="<%=path%>/analytics/oppty/stage" method="post" novalidate="true" >
			<%if(!"hidden".equals(flag)){%>
			...
			<%} %>
				...
				<div class="site-card-view">
					<div class="card-info">
						<div id="analytics_oppty_stage" style="text-align:center;color:#AAAAAA"></div>						
					</div>
				</div>
				</form>
				<%if(!"hidden".equals(flag)){ %>
