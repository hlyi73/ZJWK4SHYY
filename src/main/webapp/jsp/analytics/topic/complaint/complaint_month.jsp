...
<head>
    ...
    function initCharts(){
		...
		$.ajax({
			type : 'get',
			url : '<%=path%>/analytics/complaint/asymonth',			
	        data: dataObj,
		    success: function(data){
		    	if(data==""){
		    		$("#analytics_complaint_month").html("没有找到数据");
		    		$("#analytics_complaint_month").css("padding-top","80px");
		    		$("#analytics_complaint_month").css("min-height","200px");
		    		return;
		    	}
    ...
    function loadhigh(fact,dimession){
    	...
    	chart: {
	        renderTo: 'analytics_complaint_month',
	                ...
	                events: {
	                    click: function(e) {
	                    	...
	                    	if("true"==$(":hidden[name=flag]").val()){
 							...
 							var detail_url = "<a href='"+encodeURI("../../complaint/list?servertype=${type}&publicid=${publicId}&openid=${openId}&start_date="+month+"&end_date="+month+"&assignId="+assignerid+"&viewtype=analyticsview") +"' target='_parent'>服务请求明细</a>";
		...
		<div class="recommend-box">
			<form name="complaintForm" action="">
				...
				<div class="site-card-view">
					<div class="card-info">
						<div id="analytics_complaint_month" style="text-align:center;color:#AAAAAA;font-family:Microsoft YaHei;"></div>						
					</div>
				</div>
				</form>
