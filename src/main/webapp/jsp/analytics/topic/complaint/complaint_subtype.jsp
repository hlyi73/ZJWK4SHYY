...
<head>
    ...
    function initCharts(){
		...
		$.ajax({
			type : 'get',
			url : '<%=path%>/analytics/complaint/asytype',			
	        data: dataObj,
		    success: function(data){
		    	if(data==""){
		    		$("#analytics_complaint_subtype").html("没有找到数据");
		    		$("#analytics_complaint_subtype").css("padding-top","80px");
		    		$("#analytics_complaint_subtype").css("min-height","200px");
		    		return;
		    	}
    ...
    var chart;
    function loadhigh(fact){
    	chart = new Highcharts.Chart({
    		 chart: {
	                plotBackgroundColor: null,
	                plotBorderWidth: null,
	                plotShadow: false,
	                renderTo: 'analytics_complaint_subtype'
	            },
	            title: {
	                text: '分析报表-by类型',
	                style: {
	                ...
	                pie: {
	                	 allowPointSelect: true,
		                    ...
		                    dataLabels: {
		                        ...
		                        format: '<span style="color:blue;font-family:\'Microsoft YaHei\'">{point.name}</span><span style="color:orange;font-family:\'Microsoft YaHei\';padding-left:5px;">({point.percentage:.1f}%)</span> '
		                    },
	                    showInLegend: true,
	                    events:{
	                        click: function(e) {
	                        	var subtype = e.point.name.split("-")[0];
	                        	var startdate='',enddate='',assignerid='';
	                        	if("true"==$(":hidden[name=flag]").val()){
	 							...
	 							var detail_url = "<a href='"+encodeURI("../../complaint/list?viewtype=analyticsview&servertype=${type}&subtype="+subtype+"&publicid=${publicId}&openid=${openId}&start_date="+startdate+"&end_date="+enddate+"&assignId="+assignerid) +"' target='_parent'>服务请求明细</a>";
		...
		<div class="recommend-box">
			<form name="complaintForm" action="">
				...
				<div class="site-card-view">
					<div class="card-info">
						<div id="analytics_complaint_subtype" style="text-align:center;color:#AAAAAA;font-family:Microsoft YaHei;"></div>						
					</div>
				</div>
				</form>
