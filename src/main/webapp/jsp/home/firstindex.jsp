		    ...
		    var msgcount = 0;
	    		$(d).each(function(){
   	    		...
   	    		if(this.optype == 'contact'){
   	    			...
   	    			msgs += '<span style="color:#3e6790;">'+this.opname+'</span> 添加任务：<a href="<%=path%>/schedule/detail?openId=${openId}&publicId=${publicId}&rowId='+this.parentid+'">'+this.aftervalue+'</a></div>';
...
<body style="background-color:#fff;height:100%;">
	...
	<input type="hidden" name="taskcurrpage" value="1">
	...
	<form action="<%=path %>/home/index?openId=${openId}&publicId=${publicId}">
		...
		<c:if test="${bindSucc eq 'success' }">
			<div style="width: 100%; text-align: center;background-color:#3e6790;">
				<div class="" style="width: 100%; text-align: center;padding-top:20px;">
						<a href="<%=path%>/dcCrm/list?openId=${openId}&publicId=${publicId}">
						<!-- <div class="halo"></div> -->
						<img src="${headimgurl}"
							style="border: 0; border-radius: 60px; width: 120px; height: 120px;">
						</a>
				</div>
			
				<div
					style="width: 100%; text-align: center; margin-top: 20px;padding-bottom:20px; color: #fff">
					<span style="font-weight: bold;color:#fff; ">${nick }&nbsp;&nbsp;欢迎回来！</span>
				</div>
			</div>
