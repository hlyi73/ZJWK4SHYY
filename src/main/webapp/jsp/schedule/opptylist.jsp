...
<link rel="stylesheet" href="<%=path%>/css/page.css" id="style_color"/>
    ...
    function topage(){
		...
		$.ajax({
		      ...
		      //async: false,
		      data: {parentId:'${parentId}',viewtype:'${viewtype}',currpage:currpage,pagecount:'10'} || {},
		      dataType: 'text',
...
<body>
	...
	<div id="site-nav" class="navbar">
		...
		<h3 style="padding-right:45px;">任务列表</h3>
		<div class="act-secondary">
		<c:if test="${parentType eq 'Opportunities'}">
			<a href="<%=path%>/schedule/get?parentId=${parentId}&parentType=${parentType}&flag=other&parentName=${parentName}" style="font-size:35px;font-weight:bold;color:#fff;padding:0px 10px 0px 10px;">+</a> 
		</c:if>
		<c:if test="${parentType eq 'Accounts'}">
			<a href="<%=path%>/schedule/get?parentId=${parentId}&parentType=${parentType}&flag=other&parentName=${parentName}" style="font-size:35px;font-weight:bold;color:#fff;padding:0px 10px 0px 10px;">+</a> 
		</c:if>
		<c:if test="${parentType eq 'Project'}">
			<a href="<%=path%>/schedule/get?parentId=${parentId}&parentType=${parentType}&flag=other&parentName=${parentName}" style="font-size:35px;font-weight:bold;color:#fff;padding:0px 10px 0px 10px;">+</a> 
		</c:if>
		<c:if test="${parentType eq 'Campaigns'}">
			<a href="<%=path%>/schedule/get?parentId=${parentId}&parentType=${parentType}&flag=other&parentName=${parentName}" style="font-size:35px;font-weight:bold;color:#fff;padding:0px 10px 0px 10px;">+</a> 
		</c:if>
		</div>
	</div>
	...
	<div class="site-recommend-list page-patch ">
		...
		<div class="list-group listview tasklist" id="div_tasks_list">
			...
			<c:forEach items="${taskList }" var="task">
				<a href="<%=path%>/schedule/detail?rowId=${task.rowid}&schetype=${task.schetype}" 
