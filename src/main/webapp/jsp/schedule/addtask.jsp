...
function initDatePicker(){
	...
	$('#dateMsg').val('').scroller('destroy').scroller($.extend(opt['datetime'], {
		...
		timeWheels:'HHii', //timeWheels 显示的 时间部分
		lang: 'zh' 
	}));
	//初始化渲染日期
	$("#dateMsg").val(dateFormat(new Date(), "yyyy-MM-dd hh:mm"));
}

//初始化日期控件
function initDatePicker2(){
	var opt = {
		date : { preset : 'date', minDate: new Date(2000,3,10), maxDate: new Date(2099,7,30), stepMinute: 1  },
	};
	//类型 date  datetime     
	$('#dateMsg1').val('').scroller('destroy').scroller($.extend(opt['date'], {
		dateOrder:'yymmdd',//dateOrder 显示的日期部
		lang: 'zh' 
	}));
	//初始化渲染日期
	$("#dateMsg1").val(dateFormat(new Date(), "yyyy-MM-dd"));
}
//滚动到底部
...
function initVariable(){
    ...
    p.pub = $(".public");
    p.pubHrefs = p.pub.find("a");
    p.pubResp = $(".publicResp");
    p.pubRespStxt = p.pubResp.find(".showTxt");
    ...
    p.driority = p.taskForm.find(":hidden[name=driority]");
    ...
    p.allDay = p.taskForm.find(":hidden[name=allDay]");
    p.isPublic = p.taskForm.find(":hidden[name=ispublic]");
    p.isContinue= p.taskForm.find(":hidden[name=isContinue]");
    
    //DIV
    p.allDayDiv=$(".allDay");
    p.allDayHrefs = p.allDayDiv.find("a");
    p.allDayResp = $(".allDayResp");
    p.allDayRespStxt = p.allDayResp.find(".showTxt");
    
    //DIV
    p.continueDiv=$(".continue");
    p.continueHrefs = p.continueDiv.find("a");
    p.continueResp = $(".continueResp");
    p.continueRespStxt = p.continueResp.find(".showTxt");
    ...
    p.dateDiv1 = $(".dateDiv1");
    p.dateFlag1 = p.dateDiv1.find("input[name=dateFlag1]");
    p.dateMsg1 = p.dateDiv1.find("input[name=dateMsg1]");
    p.dateBtn1 = p.dateDiv1.find(".dateBtn1");
...
function initRelation(){
	...
	p.reTypeHrefs.click(function(){
		...
		if(k === "Activity"){
			p.reTypeSeachUrl.val('<%=path%>/campaigns/asyList');//指尖活动查询URL
			p.reTypeCurrType.val('campaignsList');
		}
		//给隐藏域赋值
		p.parentType.val(k);
...
function initTitleDate(){
	...
	//日期输入框
	p.dateBtn1.click(function(){
		var f = p.dateFlag1.val();
		var v = p.dateMsg1.val();
		if(!f || f === "startdate"){
			p.startdate.val(v);
			p.bgTimeRespStxt.html(v);
			p.bgTimeResp.css("display",'');
			p.dateFlag1.val('enddate');
		...
		if(f === "enddate"){
			...
			var end = date2utc(v); 
			if(end<start){
				p.dateMsg1.val('').attr("placeholder","结束时间不能晚于开始时间,请重新选择结束时间!");
				return;
			}
			p.enddate.val(v);
			p.endTimeRespStxt.html(v);
...
function initStatusDri(){
	...
	//状态
	p.allDayHrefs.click(function(){
		var v = $(this).attr("key"), n = $(this).html();
		p.allDay.val(v);
		p.allDayRespStxt.html(n);
		p.allDayResp.css("display",'');
		if("1"==v){
			initDatePicker2();
			p.dateDiv1.css("display",'');
		}else{
			initDatePicker();
	...
	//状态
	p.continueHrefs.click(function(){
		var v = $(this).attr("key"), n = $(this).html();
		p.isContinue.val(v);
		p.continueRespStxt.html(n);
		p.continueResp.css("display",'');
		if("1"==v){
			p.statusDiv.css("display",'');
			//滚动到底部
			scrollToButtom();
			p.total.css("display",'none');
			p.submitDiv.css("display",'none');
			//显示汇总信息
			totalMsg();
			
		}else{
			p.statusDiv.css("display",'none');
			p.statusResp.css("display",'none');
			p.cycli.css("display",'none');
			p.cycliResp.css("display",'none');
			p.contactDiv.css("display",'none');
			p.contactResp.css("display",'none');
			p.pub.css("display",'none');
			p.pubResp.css("display",'none');
			p.total.css("display",'');
			//滚动到底部
			scrollToButtom();
			//显示汇总信息
	...
	//优先级
	p.pubHrefs.click(function(){
		var v = $(this).attr("key"), n = $(this).html();
		p.isPublic.val(v);
		p.pubRespStxt.html(n);
		p.pubResp.css("display",'');
		p.contactDiv.css("display",'');
...
// 		}
	...
	p.submitTomeBtn.click(function(){
		...
		p.desc.val(d);
		p.assId.val('${crmId}');
		p.assName.val('${sessionScope.CurrentUser.name}');
		p.taskForm.submit();
	});
	//选择责任人后点确定按钮
...
function totalMsg(){
	var tmp = [];
	if(p.isContinue.val()=="1"){
		tmp=['<h1 style="font-size: 15px;">您创建的任务汇总如下所示:</h1><br>',
				 ...
				 '【5】.  是否公开: <span style="color:blue">'+ p.pubRespStxt.html() +'</span><br>',
		...
		tmp=['<h1 style="font-size: 15px;">您创建的任务汇总如下所示:</h1><br>',
			 ...
			 '【2】.  开始时间: <span style="color:blue">'+ p.startdate.val() +'</span><br>',
			 '【3】.  结束时间: <span style="color:blue">'+ p.enddate.val() +'</span><br>',
			 '【4】.  是否公开: <span style="color:blue">不公开</span><br>',
			 '【5】.  相关: <span style="color:blue">'+ p.parentTypeName.val() +'【'+ p.parentName.val() +'】</span><br>',
			 '<h2><span style="color:red">确认提交吗?</span></h2>'];
	}
	...
	<div id="taskCreate">
		...
		<div class="site-card-view ">
			...
			<form name="taskForm" action="<%=path%>/schedule/save?flag=${flag}" method="post">
				...
				<input type="hidden" name="driority" value="" /> 
...
<!-- 										<a href="javascript:void(0)" key="Activity">指尖活动 </a>&nbsp;&nbsp; -->
		...
		<div class="chatItem me titleResp" style="background: #FFF;display:none;">
			<div class="chatItemContent">
				<img class="avatar" width="35px" height="40px"
					src="${headimgurl}">
				<div class="cloud cloudText" style="margin: 0 15px 0 0;">
					<div class="cloudPannel">
						<div class="cloudBody">
		...
		<!-- 是否全天事件 -->
		<div class="chatItem you allDay" style="background: #FFF;display:none;">
			<div class="chatItemContent">
				<img class="avatar" src="<%=path%>/scripts/plugin/wb/css/images/dc.png" >
				<div class="cloud cloudText" style="margin: 0 0 0 10px;">
					<div class="cloudPannel">
								...
								<div style="word-wrap: break-word; font-family: 'Microsoft YaHei';">
									...
									<div style="margin-top: 10px;">
										<%-- <c:forEach var="s" items="${statusDom}"> --%>
											<a href="javascript:void(0)" key="1">是</a>&nbsp;&nbsp;
											<a href="javascript:void(0)" key="2">否</a>&nbsp;&nbsp;
										<%-- </c:forEach> --%>
									</div>
								</div>
							</div>
		...
		<!-- 任务状态    回复-->
		<div class="chatItem me allDayResp" style="background: #FFF;display:none;">
			<div class="chatItemContent">
				<img class="avatar" width="40px" height="40px"
					src="${headimgurl}">
				<div class="cloud cloudText" style="margin: 0 15px 0 0;">
		...
		<div class="chatItem me endTimeResp" style="background: #FFF;display:none;">
			...
			<!-- 是否完善资料 -->
		<div class="chatItem you continue" style="background: #FFF;display:none;">
			<div class="chatItemContent">
				<img class="avatar" src="<%=path%>/scripts/plugin/wb/css/images/dc.png" >
				<div class="cloud cloudText" style="margin: 0 0 0 10px;">
					<div class="cloudPannel">
		...
		<!--  是否继续完善   回复-->
		<div class="chatItem me continueResp" style="background: #FFF;display:none;">
			<div class="chatItemContent">
				<img class="avatar" width="40px" height="40px"
					src="${headimgurl}">
				<div class="cloud cloudText" style="margin: 0 15px 0 0;">
		...
		<!-- 任务优先级 -->
		<div class="chatItem you public" style="background: #FFF;display:none;">
			<div class="chatItemContent">
				<img class="avatar" src="<%=path%>/scripts/plugin/wb/css/images/dc.png" >
				<div class="cloud cloudText" style="margin: 0 0 0 10px;">
					<div class="cloudPannel">
							...
							<div class="cloudContent links">
								<div style="word-wrap: break-word; font-family: 'Microsoft YaHei';">
									...
									<div style="margin-top: 10px;">
											...
											<a href="javascript:void(0)" key="0" >不公开</a>&nbsp;&nbsp;
											<a href="javascript:void(0)" key="1" >公开</a>&nbsp;&nbsp;
									</div>
								</div>
							</div>
						</div>
		...
		<!-- 任务优先级    回复-->
		<div class="chatItem me publicResp" style="background: #FFF;display:none;">
			<div class="chatItemContent">
				<img class="avatar" width="40px" height="40px"
					src="${headimgurl}">
				<div class="cloud cloudText" style="margin: 0 15px 0 0;">
		...
		<div>
			<div class="dateDiv flooter" style="display:none;background-color:#DDDDDD;z-index:1000" >
				...
				<div class="dateDiv1 flooter" style="display:none;background-color:#DDDDDD;z-index:1000" >
				<div style="width: 100%;margin-top: 2px; margin-left: 5px; padding: 5px 100px 5px 5px;">
					<input type="hidden" name="dateFlag1" />
					<input name="dateMsg1" id="dateMsg1" value="" style="width:100%" type="text" format="yy-mm-dd" class="form-control" placeholder="点击选择日期" readonly="">
				</div>
				<div style="width:80px;float:right;margin: -47px 8px 5px 5px;">
					<a href="javascript:void(0)" class="btn btn-block dateBtn1" style="font-size: 14px;">确&nbsp;&nbsp;认</a>
				</div>
			</div>
