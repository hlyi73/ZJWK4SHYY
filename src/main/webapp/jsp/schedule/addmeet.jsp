...
function initVariable(){
    ...
    p.cycliSkip = p.cycli.find(".skip");
    
    p.maddrDiv = $(".meetingaddr");
    p.maddrResp = $(".meetingaddrResp");
    p.maddrRespStxt = p.maddrResp.find(".showTxt");
    ...
    p.addr = p.taskForm.find(":hidden[name=addr]");
...
function initSkip(){
	...
	//提醒跳 过
	p.cycliSkip.click(function(){
		p.cycliResp.css("display","none");
		p.cycliRespStxt.empty();
		//显示
		p.addrDiv.css("display","");
		p.maddrDiv.css("display","");
		//提交
...
function initTitleDate(){
	...
	p.addrBtn.click(function(){
		...
		p.addr.val(v);
		p.maddrRespStxt.html(v);
		p.maddrResp.css("display",'');
		p.total.css("display",'');
...
function totalMsg(){
	var tmp = ['<h1 style="font-size: 15px;">您创建的任务汇总如下所示:</h1><br>',
				 ...
				 '【6】.  联系人: <span style="color:blue">'+ p.contactName.val() +'</span><br>',
				 '【7】.  提醒: <span style="color:blue">'+ p.cycliValue.val() +'</span><br>',
				 '【8】.  地点: <span style="color:blue">'+ p.addr.val() +'</span><br><br>',
...
<body>
	...
	<div id="taskCreate">
		<div id="site-nav" class="navbar" >
			...
			<h3 style="padding-right:45px;">创建会议</h3>
		</div>
		<!-- 用户注册流程内容 -->
...
<!-- 										<a href="javascript:void(0)" key="Activity">指尖活动 </a>&nbsp;&nbsp; -->
								...
								<div style="word-wrap: break-word; font-family: 'Microsoft YaHei';line-height: 20px">
									...
									<div class="content" style="margin-top: 10px;">
										<c:forEach items="${reminder_time_options}" var="p">
											<a href="javascript:void(0)" key="${p.key}">${fn:replace(p.value,"提前","")}</a>&nbsp;&nbsp;
										</c:forEach>
									</div>
								</div>
							</div>
		...
		<!-- 会议地点 -->
		<div class="chatItem you meetingaddr" style="background: #FFF;display:none;">
			<div class="chatItemContent">
				<img class="avatar" src="<%=path%>/scripts/plugin/wb/css/images/dc.png" >
				<div class="cloud cloudText" style="margin: 0 0 0 10px;">
					<div class="cloudPannel">
		...
		<!-- 任务主题    回复-->
		<div class="chatItem me meetingaddrResp" style="background: #FFF;display:none;">
			<div class="chatItemContent">
				<img class="avatar" width="40px" height="40px"
					src="${headimgurl}">
				<div class="cloud cloudText" style="margin: 0 15px 0 0;">
		...
		<div>
			...
			<div class="txtDiv flooter" style="display:none;background-color:#DDDDDD;" >
				<div style="width: 100%;margin-top: 2px; margin-left: 5px; padding: 5px 100px 5px 5px;">
					<input name="txtMsg" value="" style="width:100%" type="text" class="form-control" placeholder="输入会议主题">
				</div>
				<div style="width:80px;float:right;margin: -47px 8px 5px 5px;">
			...
			<div class="addrDiv flooter" style="display:none;background-color:#DDDDDD;" >
				<div style="width: 100%;margin-top: 2px; margin-left: 5px; padding: 5px 100px 5px 5px;">
					<input name="addrMsg" value="" style="width:100%" type="text" class="form-control" placeholder="输入会议地点">
				</div>
				<div style="width:80px;float:right;margin: -47px 8px 5px 5px;">
