...
function initDatePicker(){
	...
	$("#dateMsg").val(dateFormat(new Date(), "yyyy-MM-dd")+ " 09:00:00");
}
...
function initTitleDate(){
	...
	p.dateBtn.click(function(){
		...
		if(!f || f === "startdate"){
			...
			$("#dateMsg").val(dateFormat(new Date(), "yyyy-MM-dd")+ " 18:00:00");			
...
//汇总信息
function totalMsg(){
	var tmp = ['<h1 style="font-size: 15px;">您填写的日报信息所示:</h1><br>',
...
<body>
	...
	<div id="taskCreate">
		<div id="site-nav" class="navbar" >
			...
			<h3 style="padding-right:45px;">创建日报</h3>
		</div>
		<!-- 用户注册流程内容 -->
		<div class="site-card-view ">
			...
			<form name="taskForm" action="<%=path%>/schedule/save?flag=${flag}" method="post">
				...
				<input type="hidden" name="schetype" value="report" />
...
<!-- 										<a href="javascript:void(0)" key="Activity">指尖活动 </a>&nbsp;&nbsp; -->
								...
								<div style="word-wrap: break-word; font-family: 'Microsoft YaHei';">
									...
									<div style="margin-top: 10px;">
										<c:forEach var="s" items="${statusDom}">
											<c:if test="${s.key eq 'Completed' || s.key eq 'Suspend' || s.key eq 'In Progress' || s.key eq 'Deferred' }">
		...
		<div>
			...
			<div class="txtDiv flooter" style="display:none;background-color:#DDDDDD;" >
				<div style="width: 100%;margin-top: 2px; margin-left: 5px; padding: 5px 100px 5px 5px;">
					<!-- <input name="txtMsg" value="" style="width:100%" type="text" class="form-control" placeholder="输入工作内容"> -->
					<textarea rows="2" name="txtMsg" style="width:100%" type="text" class="form-control" placeholder="输入工作内容" ></textarea>
				</div>
				<div style="width:80px;float:right;margin: -47px 8px 5px 5px;">
					...
					<fieldset class="">
						<div class="ui-block-a" style="width:100%;">
							<a href="javascript:void(0)" class="btn btn-block submitTomeBtn" 
							    style="font-size: 16px;margin-left:10px;margin-right:10px;">保存</a>
						</div>
					</fieldset>
				</div>
			</div>
