...
function initVariable(){
    ...
    p.reTypeFstChar = p.reList.find(":hidden[name=fstChar]");
	p.reTypeSeachUrl = p.reList.find(":hidden[name=seachUrl]");
	p.reTypeCurrType = p.reList.find(":hidden[name=currType]");
	p.reTypeCurrPage =p.reList.find(":hidden[name=currPage]");
	p.reTypePageCount = p.reList.find(":hidden[name=pageCount]");
...
function initRelation(){
		p.reTypeSeachUrl.val('<%=path%>/project/asyList');
		p.reTypeCurrType.val('projectList');		
		//给隐藏域赋值
		//p.parentType.val('Project');
		p.parentTypeName.val('');
		p.reTypeFstChar.val(''); 
...
function initParent(){
	...
	p.parentType.val('Project');//计划任务的parentType 都为 Project;赋值给隐藏域
}
...
<body>
	...
	<div id="taskCreate">
		<div id="site-nav" class="navbar" >
			...
			<h3 style="padding-right:45px;">创建计划任务</h3>
		</div>
		<!-- 用户注册流程内容 -->
		<div class="site-card-view ">
			...
			<form name="taskForm" action="<%=path%>/schedule/save?flag=${flag}" method="post">
				...
				<input type="hidden" name="schetype" value="plan" />
...
<%-- 		<div class="chatItem you relationType" style="background: #FFF;">
		...
		<!-- 任务相关列表 -->
		<div class="chatItem you relationList" style="background: #FFF;display:'';">
			<div class="chatItemContent">
			    <img class="avatar" src="<%=path%>/scripts/plugin/wb/css/images/dc.png">
				<div class="cloud cloudText" style="margin: 0 0 0 10px;">
					<div class="cloudPannel">
