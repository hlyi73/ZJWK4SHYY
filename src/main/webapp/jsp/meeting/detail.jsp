...
/* 	function initWeixinFunc(){
    ...
    function initControl(){
    	var status = '${sd.expensestatus}',//审批状态 新建 待审批 已批准 驳回
    	    ...
    	    crmId = '${crmId}';//查看界面的用户
    	    commeitId = $(".examineInfo tr:last").attr("id"), //最后一个提交人
    	    lststatus = $(".examineInfo tr:last").attr("st");//最后一个提交人 的审批状态
    	...
    	}else if(status === "approving" && auditor === crmId){//待审批两种情况:  1.审批申请人查看   2.审批人进行审批的查看
    ...
    function initExaminerEle(){
    	var ec = $(".examiner"),
    	    ...
    	    //外层消息发送按钮
    	    examinerSend = msgc.find(".examinerSend"),
    	    ...
    	    //消息发送输入框
    	    inputMsgInput = msgc.find("input[name=inputMsg]");
    	    
    	//发送消息
    	examinerSend.click(function(){
    		sendOpptyMsg(inputMsgInput);
    		inputMsgInput.val('');
    	});
    	//外层审批按钮
    ...
    //发送业务机会回复消息
    function sendOpptyMsg(obj){
    	var v = obj.val();
    	if(v && v.length > 255){
    		obj.attr("placeholder", "输入的字数过长!");
    		return;
    	}
    	var dataObj = [];
    	dataObj.push({name:'userId', value: '${crmId}'});
    	dataObj.push({name:'username', value: '${assigner}'});
    	dataObj.push({name:'content', value: v});
    	dataObj.push({name:'relaModule', value: 'expense'});
    ...
    function showMessage(c, p){
    	...
    	dataObj.push({name:'pagecount', value:p || '5'});
 		   ...
 		   success: function(data){
 			  ...
 			  if(d){
			      ...
			      $(d).each(function(){
				      ...
				      var html= '';
				      html += '<li  style="border-bottom: #eff2f5 dashed 1px;display: block;position: relative;padding: 10px 0;">';
				      html += '  <div class="ct-box" style="display: block;margin-left: 0px;">';
				      html += '  <p class="ct-user" style="margin-bottom: 6px;">';
				      html += '    <a target="_blank" href="javascript:void(0)">'+ this.username +'</a> :';
    ...
    function topageOppty(){
		...
		$.ajax({
		      ...
		      //async: false,
		      data: {viewtype:'myallview',currpage:currpage,publicId:'${publicId}',openId:'${openId}',pagecount:'10'} || {},
		      dataType: 'text',
		      success: function(data){
		...
		$.ajax({
		      ...
		      success: function(data){
					...
					if(d != ""){
						...
						$(d).each(function(i){
							val += '<a href="<%=path%>/customer/detail?rowId='+this.rowid+'&openId=${openId}&publicId=${publicId}" class="list-group-item listview-item radio">'
		...
		$.ajax({
		      ...
		      success: function(data){
					...
					if(d != ""){
						...
						$(d).each(function(i){
							val += '<a href="<%=path%>/schedule/detail?rowId='+this.rowid+'&openId=${openId}&publicId=${publicId}" class="list-group-item listview-item radio">'
	...
	function topageProject(){
		...
		$.ajax({
		      ...
		      //async: false,
		      data: {viewtype:'myallview',currpage:currpage,crmId:'${crmId}',pagecount:'10'},
		      dataType: 'text',
		      success: function(data){
...
<body>
	<div id="site-nav" class="navbar">
	    <a href="javascript:void(0)" style="display:none" class="act-primary goBack"><i class="icon-back"></i></a>
...
<!-- 			<i class="icon-menu"><b></b></i> -->
	...
	<div class="view site-recommend detailInfo">
		<div class="recommend-box">
			...
			<form name="expenseForm" action="<%=path%>/expense/save" method="post" novalidate="true">
					...
					<div class="card-info">
						<table>
							<tbody>
									...
									<td class="uptInput" style="display:none">
										<select name="expensetypeSel" onchange="updateExpenType()" style="height:2.2em">
									       <c:forEach var="item" items="${expenseTypeList}" varStatus="status">
												<c:if test="${item.value eq sd.expensetype}">
													<option value="${item.key}" selected>${item.value}</option>
												</c:if>
												<c:if test="${item.value ne sd.expensetype}">
													<option value="${item.key}" >${item.value}</option>
												</c:if>
											</c:forEach>
										</select>
								...
								<tr>
									<th>金额：</th>
									<td class="uptShow">￥${sd.expenseamount}</td>
									<td class="uptInput" style="display:none">
							...
							<dl class="hyrc" id="tc01">
								<c:forEach items="${approList }" var="app">
									<c:if test="${app.approvalstatus eq 'approving'}">
										...
										<dd style="width:60%;padding-right:5px;">
											<p>
									...
									<c:if test="${app.approvalname ne ''}">
										...
										<dd style="width:auto;padding-right:5px;">
											<p>
												等待&nbsp;<span style="color:#3e6790">${app.approvalname}</span>&nbsp;审批
											</p>
										</dd>
									</c:if>
									 -->
									...
									<c:if test="${app.approvalstatus eq 'reject'}">
										...
										<dd style="width:60%;padding-right:5px;">
											<p>
												<span style="color:#3e6790">${app.commitname}</span>&nbsp;<span style="color:red">已驳回</span>
												<c:if test="${app.approvaldesc ne '' }">
													(驳回原因：${app.approvaldesc})
												</c:if>
											</p>
										</dd>
									</c:if>
				...
				<!-- 消息回复 -->
				<h3 class="wrapper examineInfo">消息回复</h3>
				<div  class="container hy bgcw msgListContainer" style="font-size: 14px">
					<ul style="margin-top: 20px;border-top: #eff2f5 dashed 1px;"></ul>
				</div>
				<div  class="container hy bgcw msgListShowAll" style="font-size: 14px;text-align: center;padding-top: 5px;padding-bottom: 5px">
					<a class="showAllMsg" style="cursor: pointer;color: #51BBEC;padding-top: 5px;padding-bottom: 5px;" onclick="showAllMessage();" href="javascript:void(0)">查看全部&nbsp;↓</a>
				</div>
				...
				<div class="examiner" style="display:none;margin-top:5px;text-align:center;">
					...
					<div class="flooter" style="z-index: 99999;background: #FFF;background: rgb(242, 242, 243);border-top: 1px solid #A2A2A2;opacity: 1;">
						...
						<div class="msgContainer">
							<div class="ui-block-a " style="float: left;width: 20%;margin: 5px 0px 5px 0px;">
								<a href="javascript:void(0)"  class="btn btn-block examinerBtn" style="font-size: 14px;padding:0px;margin-left: 5px;">审批</a>
							</div>
							<div class="ui-block-a " style="float: left;width: 65%;margin: 5px 0px 5px 0px;">
								<input name="inputMsg" value="" style="width:98%;font-size:14px;line-height:40px;height:40px;margin-left: 5px;margin-top: 0px;" type="text" class="form-control" placeholder="回复">
							</div>
							<div class="ui-block-a " style="float: left;width: 15%;margin: 5px 0px 5px 0px;">
								<a href="javascript:void(0)"  class="btn  btn-block examinerSend" style="font-size: 14px;padding:0px;margin-left: 5px;margin-right: 5px;">发送</a>
							</div>
							<div style="clear: both;"></div>
						</div>
						<!--按钮的区域  -->
						<div class="btnContainer" style="display: none">
							<!-- <div class="ui-block-a agreeAnd2Next" id="agreeAnd2Next" style="margin: 10px 12px 10px 12px;padding-bottom: 5px;font-family: 'Microsoft YaHei';padding-bottom: 5px;border-bottom: 1px dashed #B198B6;color: #878C91;font-size: 14px;">
								<span>请&nbsp;&nbsp;选&nbsp;&nbsp;择&nbsp;&nbsp;审&nbsp;&nbsp;批&nbsp;&nbsp;类&nbsp;&nbsp;别：</span>
							</div> -->
							...
							<div class="ui-block-a cannel" style="margin: 10px 12px 10px 12px;padding-bottom: 5px;font-family: 'Microsoft YaHei';color: #878C91;font-size: 14px;cursor: pointer;">
								<span>取             &nbsp;消</span>
							</div>
						</div>
						<!--不同意按钮的区域  -->
						<div class="refuseContainer" style="display: none">
							<div class="ui-block-a " style="margin: 10px 12px 10px 12px;padding-bottom: 5px;font-family: 'Microsoft YaHei';padding-bottom: 5px;border-bottom: 1px dashed #B198B6;color: #878C91;font-size: 14px;f">
								<li style="text-align: left">请填写不同意原因：</li>
							</div>
							<div class="ui-block-a " style="margin-left: 12px">
							    <textarea  name="expense_description" id="expense_description" style="width:98%" rows="3"  placeholder="不同意的原因，可选" class="form-control"></textarea>
							</div>
							<div class="ui-block-a " id="" style="margin: 10px 12px 10px 12px;padding-bottom: 5px;">
								<li href="javascript:void(0)"  
								     class="btn  btn-block btn-danger refuseSubmit" 
								       style="width: 48%;float:left;font-size: 14px;padding:0px;margin-bottom: 5px;" onclick="approExam('n')">提&nbsp;&nbsp;交</li>
								<li href="javascript:void(0)"  
								     class="btn  btn-block refuseCannel" 
								       style="width: 48%;float:right;font-size: 14px;padding:0px;margin-bottom: 5px;">取&nbsp;&nbsp;消</li>
							</div>
						</div>
					</div>
				</div>
	...
	<div class="site-recommend-list page-patch opptyListDiv" style="display:none">
		...
		<div class="list-group listview" id="div_oppty_list"></div>
	...
	<div class="site-recommend-list page-patch projectListDiv" style="display:none">
		...
		<div class="list-group listview " style="margin-top:-1px;" id="div_project_list"></div>
	...
	<div class=" userList" style="display:none;padding-bottom:50px;"	>
		...
		<!-- 报销申请人 提交 -->
		<div class=" commitExamDiv flooter" style="">
			<a class="btn btn-block " type="button" onclick="commitExam();"
			     style="font-size: 14px;height:2.8em;line-height: 2.8em">确&nbsp定</a>
		</div>
		<!-- 报销审批人 提交 -->
		<div class="nextCommitExamDiv flooter" style="display:none">
			<div class="button-ctrl">
				<a href="javascript:void(0)"  class="btn btn-block"
								style="font-size: 14px;" onclick="approEndExam()">提交</a>
			</div>
		</div>
	</div>
	<!-- myMsgBox -->
