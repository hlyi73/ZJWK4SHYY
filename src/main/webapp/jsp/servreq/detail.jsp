...
<script src="<%=path%>/scripts/model/servreq_detail.model.js"></script>
<!--css框架样式-->
...
$(function(){
	serveq_detail.openid = '${openid}';
	serveq_detail.publicid = '${publicid}';
	serveq_detail.crmid = '${crmid}';
	serveq_detail.parentid = '${rowid}';
	serveq_detail.servertype = '${sd.servertype}';
	serveq_detail.parentname = '${sd.name}';
	serveq_detail.status = '${sd.status}';
...
<body>
	<!--头部 -->
	<div id="site-nav" class="navbar serveq_nav">
	...
	<!-- 业务机会详情 容器 -->
	<div id="servreq_detail_con" class=" view site-recommend recommend-box ">
		<!-- 简介区域 -->
			...
			<div class="card-info">
				<a href="<%=path%>/complaint/modify?servertype=${sd.servertype}&crmid=${crmid}&openid=${openid}&publicid=${publicid}&rowid=${rowid}"
					...
					<div class="list-group-item-bd">
						<h1 class="title">${sd.case_number}&nbsp;
						</h1>
						<p> 状态：${sd.status_name}</p>
						<p> 受理人：${sd.handle_name}</p>
						<p> 关闭日期：${sd.finish_time}</p>
		...
		<div class="list list-group listview accordion count_con"
			...
			<div style="float:left;width:28%;text-align:center;">
				<a href="<%=path%>/complaint/serexeclist?parentid=${rowid}&parenttype=${servertype}&openid=${openid}&publicid=${publicid}"
				class="list-group-item-bd"> 
					<div style="float:left;text-align:right;width:50%;"><img src="<%=path%>/image/wx_contact.png" width="35px" border=0></div>
					<div style="float:right;text-align:left;width:50%;">
						<div id="exec_count" style="height:16px;padding-left:15px;">0</div>
						<div style="height:16px;padding-left:5px;font-size:12px;">执行</div>
					</div>
				</a>
			</div>
			<!-- 服务回访 -->
			<div style="float:left;width:24%;text-align:center;">
				<a
				href="<%=path%>/complaint/servisitlist?parentid=${rowid}&openid=${openid}&publicid=${publicid}"
				class="list-group-item-bd"> 
	...
	<div id="flootermenu" class="flooter" style="z-index: 99999; background: #FFF;border-top: 1px solid #ddd; opacity: 1;height:51px">
		...
		<div class="add_container" style="background:#fff;z-index:99999;height:0px;border-top-color: #E6DADA;border-top: 1px solid #C2B2B2;padding-top: 25px;">
			<div class="ui-block-a servpatch_add_btn" style="cursor:pointer;float: left; width: 25%; margin-left: 10px;margin: 0px 0px 5px 0px;">
				...
				<div style="font-size: 13px;margin-top: 10px;color: #8D8282;font-family: 'Microsoft YaHei';">服务派工</div>
				<div style="line-height:20px;">&nbsp;</div>
			</div>
			<div class="ui-block-a exec_add_btn" style="background:#fff;cursor:pointer;float: left; width: 25%; margin-left: 10px;margin: 0px 0px 5px 0px;">
				<div style="border-radius: 5px;border: 1px solid #E6DFDF;margin-left: 10px;margin-right: 10px;">
					<img alt="" src="<%=path %>/image/oppty_events.png" style="width: 35px;height: 35px;margin-top: 10px;margin-bottom: 10px;">
				</div>
				<div style="font-size: 13px;margin-top: 10px;color: #8D8282;font-family: 'Microsoft YaHei';">服务执行</div>
				<div style="line-height:20px;">&nbsp;</div>
			</div>
			<div class="ui-block-a servisit_add_btn" style="background:#fff;cursor:pointer;float: left; width: 25%; margin-left: 10px;margin: 0px 0px 5px 0px;">
				...
				<div style="font-size: 13px;margin-top: 10px;color: #8D8282;font-family: 'Microsoft YaHei';">服务回访</div>
				<div style="line-height:20px;">&nbsp;</div>
			</div>
			...
			<div class="ui-block-a task_add_btn" style="cursor:pointer;float: left; width: 25%; margin-left: 10px;margin: 5px 0px 5px 0px;">
				<div style="border-radius: 5px;border: 1px solid #E6DFDF;margin-left: 10px;margin-right: 10px;">
	...
	<jsp:include page="/common/trackhis.jsp">
		...
		<jsp:param name="callback_p2pmsg_seleted"  value="callback_servreq_p2pmsg_seleted"/>
	</jsp:include>
	...
	<jsp:include page="/common/team.jsp">
		...
		<jsp:param name="main_invoke_con" value="servreq_detail_con"/>
