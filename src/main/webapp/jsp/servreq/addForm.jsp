...
<script type="text/javascript">
		...
		function initDatePicker(){
			...
			//类型 date  datetime
			$('#propose_time').val(dateFormat(new Date(), "yyyy-MM-dd")).scroller('destroy').scroller($.extend(opt['date'], { theme: 'default', mode: 'scroller', display: 'modal', lang: 'zh' }));
		...
		function initForm(){
			...
			$(".assignerbtn").click(function(){
				$(".assignerList > a.checked").each(function(){
					...
					if("true"==$(":hidden[name=flag]").val()){
						...
						$(":hidden[name=sponsor]").val(assId);
						$(".assignerGoBak").trigger("click");
					}else{
						$(":hidden[name=handle]").val(assId);
						if(!$(":hidden[name=handle]").val()){
							$(".myMsgBox").css("display","").html("受理人不能为空,请您选择受理人!");
			...
			$(".submitBtn").click(function(){
				$(":hidden[name=handle]").val('${assignerid}');
				if(!validates()){
					$("form[name=complaintform]").submit();
				}
			});
	    ...
	    function topage(type){
	  	...
	  	function validates(){
  			...
  			var case_number = $("input[name=case_number]");
  			var reg=/^[0-9]{5}$/;
  			if(!reg.test(case_number.val())){
  				$(".myMsgBox").css("display","").html("请输入正确的五位数字编号!");
    	    	$(".myMsgBox").delay(2000).fadeOut();
  				return true;
  			}
	...
	<div id="task-create" class=" ">
		...
		<div class="wrapper">
			<form id="complaintform" name="complaintform" action="<%=path%>/complaint/save" method="post"  >
				...
				<input type="hidden" name="handle_date" value="${date}" />
				<input type="hidden" name="handle"/>
				<!-- 服务请求信息编号 -->
				<div class="form-group">
					<label class="control-label" >服务请求信息编号[00000-99999]&nbsp;<span style="color:red;">*</span></label>
					<input name="case_number" id="case_number" value="" type="number"
						class="form-control" placeholder="请输入五位数字编号"/>
				</div>
				<!-- 相关客户 -->
				<div class="form-group">
				...
				<!-- 相关合同 -->
				<div class="form-group">
					<label class="control-label" for="parent">合同&nbsp;<span style="color:red;">*</span></label>
				...
				<!-- 服务所在地点 -->
				<div class="form-group">
					<label class="control-label" for="realname">服务所在地点&nbsp;<span style="color:red;">*</span></label>
					<input name="position" id="position" type="text"
						class="form-control" placeholder="请输入服务所在地点" />
				</div>
				<!-- 要求服务日期 -->
				<div class="form-group">
					<label class="control-label" for="startdate">要求服务日期&nbsp;<span style="color:red;">*</span></label>
					<input name="propose_time" id="propose_time" type="text" format="yy-mm-dd" 
						class="form-control" placeholder="要求服务日期" readonly="" />
				</div>
				<!-- 服务状态 -->
				<div class="form-group">
					<label class="control-label" for="status">服务状态&nbsp;<span style="color:red;">*</span></label>
				...
				<!-- 服务分类 -->
				<div class="form-group">
					<label class="control-label" for="driority">服务分类&nbsp;<span style="color:red;">*</span></label>
					<div class="form-control select">
						<div class="select-box"></div>
						<select name="subtype" id="subtype">
							<c:forEach var="item" items="${substypes_servq}">
								<option value="${item.key}">${item.value}</option>
							</c:forEach>
						</select>
					</div>
				...
				<!-- 发起人 -->
				<div class="form-group">
					<label class="control-label" for="assigner">发起人&nbsp;<span style="color:red;">*</span></label>
					<input name="sponsor" id="sponsor" type="hidden" class="form-control" value="${sessionScope.assignerid}" >
					<input name="sponsorname" id="sponsorname" val="Sponsor" type="text"  value="${sessionScope.assigner}"
					       class="form-control assignerChoose" placeholder="【点击  选择发起人】  >>  " readonly="readonly" >
				</div>
				<!-- 客户诉求 -->
				<div class="form-group">
					<label class="control-label" for="participant">客户诉求&nbsp;<span style="color:red;">*</span></label>
					<textarea name="name" id="name" rows="4" style="min-height: 3em" 
					       class="form-control" placeholder="请输入客户诉求" ></textarea>
				</div>
				<!-- 主要产品,可填 -->				
				<div class="form-group">
					<label class="control-label" for="desc">主要产品</label>
					<textarea name="productname" style="min-height: 3em" id="desc" rows="4"
						class="form-control" placeholder="主要产品,可填"></textarea>
				</div>
					...
					<fieldset class="">
						...
						<div class="ui-block-a">
							<a href="javascript:void(0)" class="btn btn-block submitToOtherBtn" 
							    style="font-size: 16px;margin-left:10px;margin-right:10px;">派工</a>
						</div>
					</fieldset>
				</div>
			</form>
