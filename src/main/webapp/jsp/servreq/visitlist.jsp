...
<body>
	...
	<div id="site-nav" class="navbar">
		...
		<div class="act-secondary">
			<%-- <a href="<%=path%>/complaint/visit_choose?openid=${openid}&publicid=${publicid}" style="font-size:35px;font-weight:bold;color:#fff;padding:0px 10px 0px 10px;">+</a> --%> 
		</div>
		<!-- 下拉选择框 -->
				...
				<div class="list-group-item-bd" style="width:180px;margin:0 auto;padding-top:5px;" >
					<p>
						<div class="form-control select _viewtype_select">
							<div class=" viewtypelabel" style="color:white"><h3>服务回访列表</h3></div>
						</div>
					</p>
				</div>
			</form>
	...
	<div class="site-recommend-list page-patch ">
		<div class="list-group listview compslist" style="margin-top:-1px;" id="div_tasks_list">
			<c:forEach items="${visitlist}" var="visit">
				<a href="<%=path%>/complaint/servisitdetail?rowid=${visit.rowid}&parentid=${visit.parentid}&openid=${openid}&publicid=${publicid}&crmid=${crmid}" 
					class="list-group-item listview-item">
					<div class="list-group-item-bd">
						<div class="content">
							<h1>${visit.assigned_user_name}</h1>
							<p class="text-default">回访日期: ${visit.visit_date}</p>
		...
		<!-- 翻页 -->
		<c:if test="${fn:length(visitlist)==10 }">
