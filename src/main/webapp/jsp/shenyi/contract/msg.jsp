		...
		<div class="view site-phonebook">
			<div class="well text-center wrapper" style="padding: 2em 0">
				<c:if test="${success eq 'ok'}">
				    <h3 class="text-info" style="font-size: 18px;">创建合同成功!</h3>
				    <br>合同已同步到CRM<br>您同时可以登陆CRM系统中查看！<br><br>
				      <form name="detailForm" action="<%=path%>/contract/detail" method="post" novalidate="true" >
				         <input type="hidden" name="rowId" value="${rowId}" />
				         <input type="hidden" name="publicId" value="${publicId}" />
				         <input type="hidden" name="openId" value="${openId}" />
				         <a href="javascript:void(0)" class="btn customerDetail">
					        <span class="icon icon-search "></span> 点击查询详情
					    </a>
				    </form>
				</c:if>
				<c:if test="${success eq 'fail'}">
				    <h3 class="text-info" style="font-size: 18px;color:red">创建合同失败!</h3>
