...
<!-- 			<i class="icon-menu"><b></b></i> -->
		...
		<a href="javascript:void(0)" class="list-group-item listview-item">
		  <form name="viewtypeForm" action="<%=path%>/gathering/verlist?openId=${openId}&publicId=${publicId}" method="post">
					...
					<div class="form-control select">
						...
						<div class="select-box viewtypelabel">我的待回款列表</div>
							<select name="viewtypesel" id="viewtypesel">
								<option value="myview_wait">待回款列表</option>
								<option value="myview_notreceive">未及时回款列表</option>
								<option value="allview_received">已回款列表</option>
								<option value="allview_applied">已核销列表</option>
							</select>
						</c:if>
					</div>
				</p>
		...
		<div class="site-recommend-list page-patch">
			<div id="div_gathering_list" class="list-group listview">
				<c:forEach items="${gatheringList}" var="gath">
					<a href="<%=path%>/gathering/detail?rowId=${gath.rowid}&openId=${openId}&publicId=${publicId}&verifityFlag=verifity"
						class="list-group-item listview-item">
						<input type="hidden" name="sglRowId" value="${gath.rowid}" ><!-- rowID -->
						<div class="list-group-item-bd">
							...
							<div class="content" style="text-align: left">
								...
								<p class="text-default">计划:&nbsp;&nbsp;${gath.planDate}&nbsp;&nbsp;￥${gath.planAmount}</p>
								<span>
									<p class="text-default">实际:&nbsp;&nbsp;${gath.receivedDate}&nbsp;&nbsp;￥${gath.receivedAmount}</p>
