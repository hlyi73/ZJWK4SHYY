	...
	<div id="expense-create">
		...
		<div id="gotodiv" style="position:absolute;top:40%;left:35%;z-index:999;background-color:#EEEEEE;padding:2px;display:none;">
			...
			<div style="background-color:#FFF;padding:10px 20px 10px 20px;">
				<a href="javascript:void(0)" style="font-size: 14px;" onclick="finaVerRst('y')">核销通过</a>
			</div>
			<div style="border-bottom:1px solid #EEEEEE;background-color:#FFF;padding:10px 20px 10px 20px;">
				<a href="javascript:void(0)" style="font-size: 14px;" onclick="finaVerRst('n')">核销不通过</a>
			</div>
			
			<form action="<%=path %>/gathering/finaVer" name="verifityForm" >
				    <input type="hidden" name="verifityDate" /> <!--核销日期-->
				    <input type="hidden" name="verStatus" />  <!--核销状态-->
				    <input type="hidden" name="verifityRst" /> <!-- 核销说明 -->
				    <input type="hidden" name="verifityAmount" /><!-- 核销金额 -->
				    <input type="hidden" name="rowId" value="${rowId}" />
				    <input type="hidden" name="publicId" value="${publicId}" /> 
					<input type="hidden" name="openId" value="${openId}" /> 
					<input type="hidden" name="invoicedId" /><!-- ID-->
			    </form>
			
		</div>
	
		<div id="gatheringInfo" style="display:none;">
		...
		<div class="site-card-view bxFlowContent">
			<!-- 提交报销数据的表单 -->
			<form name="followupForm" action="<%=path%>/gathering/saveFollowup" method="post">
			    <input type="hidden" name="publicId" value="${publicId}" /> 
				<input type="hidden" name="openId" value="${openId}" /> 
				<input type="hidden" name="rowId" value="${rowId}" />
				<input type="hidden" name="planDate" value="${obj.planDate}" /><!-- 应收日期 -->
				<input type="hidden" name="planAmount" value="${obj.planAmount}" /><!-- 应收金额 -->
				<input type="hidden" name="receivedDate" /><!-- 实收日期 -->
				<input type="hidden" name="receivedAmount" value="${obj.receivedAmount}"/><!-- 实收金额 -->
				<input type="hidden" name="margin" value="${obj.margin}"/><!-- 差额 -->
				<input type="hidden" name="desc" /><!-- 描述-->
				<input type="hidden" name="modifydate" /><!-- 修改日期-->
				<!-- 财务核销 -->
				<input type="hidden" name="invociedDate" /><!-- 开票日期-->
				<input type="hidden" name="invoicedAmount" /><!-- 开票日期-->
				<input type="hidden" name="verStatus" /><!-- 状态-->
				<input type="hidden" name="invoicedId" /><!-- ID-->
			</form>
		</div>
