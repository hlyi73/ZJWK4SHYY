...
<script type="text/javascript">
	...
	function calBatchDel(){
		...
		$("#div_rival_list a").removeClass("checked");
		$(".approveTip").html('');
	}
	
	//初始化全选和不全选按钮
	function initDelBtn(){
		$("#div_rival_list > a").unbind("click").bind("click", function(){
		...
		$(".checkAllBtn").unbind("click").bind("click", function(){
			if($(this).parent().hasClass("checked")){
				...
				$("#div_rival_list a").addClass("checked");
			}
			return false;
		});
		//删除
	...
	function getRowIds(){
		var rowids = '';
		$("#div_rival_list a.checked").each(function(i){
...
<body>
	<div id="site-nav" class="navbar">
		...
		<h3 style="padding-right:45px;">对手列表</h3>
		<div class="act-secondary">
			<a href="<%=path%>/rival/get?opptyid=${opptyid}&openId=${openId}&publicId=${publicId}" style="font-size:35px;font-weight:bold;color:#fff;padding:0px 10px 0px 10px;">+</a> 
		</div>
		...
		<div class="site-recommend-list page-patch">
			<div id="div_rival_list" class="list-group listview">
			...
			<a href="javascript:void(0)" class="list-group-item listview-item  radio batchDelHref flooter" style="display:none;padding:.3em;padding-bottom: .3em;">
			    	<form name="batchDelForm" method="post" action="<%=path%>/rival/del?openId=${openId}&publicId=${publicId}">
			    		<!-- 审批字段 -->
...
<!-- 					<div class="approveTip" style="color:red;margin-right:15px"></div> -->
					<div class="input-radio checkAllBtn" title="全选"></div>
				</a>
			
				<c:forEach items="${rivalList}" var="rival">
					<a href="<%=path%>/customer/detail?rowId=${rival.customerid}&flag=rival&openId=${openId}&publicId=${publicId}"
						class="list-group-item listview-item radio">
						<input type="hidden" name="sglRowId" value="${rival.rowid}" ><!-- rowID -->
						<div class="list-group-item-bd">
							<div class="content" style="text-align: left">
								<h1>${rival.customername}</h1>
							</div>
						</div>
				...
				<c:if test="${fn:length(rivalList) == 0 }">
