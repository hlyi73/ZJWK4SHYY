...
$(function(){
	...
	um.setContent('${activity.content}');

	//初始化左侧图片
	$(".js_appmsg_thumb").removeClass("appmsg_thumb");
	$(".js_appmsg_thumb").css("width","100%");
	$(".js_appmsg_thumb").attr("src",'<%=path%>/mkattachment/download?fileName=${activity.logo}&flag=headImage');
...
function getData(){
	...
	$("form[name=activityform]").find(":hidden").each(function(){
		...
		var value  = $(this).val();
		if('id'==name||'openId'==name||'source'==name||'sourceid'==name||'logo'==name||'remark'==name||'content'==name){
...
<!-- 菜单 -->
	...
	<div class="dashboard-wrapper" >
		<!-- Left Sidebar Start -->
			<div class="main_bd" style="margin-left: 100px;margin-top: 50px;">    
			<div class="media_preview_area">        
			    <div class="appmsg editing">            
			      <div id="js_appmsg_preview" class="appmsg_content">                                            
			        <div id="appmsgItem1" data-fileid="" data-id="1" class="js_appmsg_item">
				        <h4 class="appmsg_title" style="max-height: 60px;"><a onclick="return false;" class="title" href="javascript:void(0);" target="_blank">${activity.title }</a></h4>
	    	...
	    	<form id="activityform" name="activityform" action="<%=path%>/zjwkactivity/update" method="post"  >
			    <input type="hidden" name="openId" value="${sourceid}" /> 
				<input type="hidden" name="sourceid" value="${sourceid}" /> 
				<input type="hidden" name="source" value="${source}" /> 
				<input type="hidden" name="orgId" value="${orgId}" />
				<input type="hidden" name="logo" value="${activity.logo }" />
				<input type="hidden" name="remark" value="${activity.remark}" />
 				...
 				<div class="inner">
					<div class="appmsg_edit_item">
						...
						<span class="frm_input_box">
							<input type="text" onkeyup="cloneText(this);"value="${activity.title}" name="title" class="frm_input js_title">
						</span>
					</div>
					<div class="appmsg_edit_item">
						<label for="" class="frm_label">活动地址&nbsp;<span style="color:red;">*</span></label>
						<span class="frm_input_box">
							<input type="text" name="place" value="${activity.place}"class="frm_input js_author ">
						</span>
					</div>
					<div class="appmsg_edit_item">
						...
						<span class="frm_input_box" style=" background-color: #efefef;">
							<jsp:include page="/common/marketing/datetime.jsp">
							    ...
							    <jsp:param name="defvalue" value="${activity.start_date }" />
							</jsp:include>
						</span>
					</div>
					<div class="appmsg_edit_item">
						...
						<span class="frm_input_box" style=" background-color: #efefef;">
							<jsp:include page="/common/marketing/datetime.jsp">
							    ...
							    <jsp:param name="defvalue" value="${activity.end_date }" />
							</jsp:include>
						</span>
					</div>
					<div class="appmsg_edit_item">
						...
						<select class="frm_input_box" style="width:100%"name="charge_type" id="charge_type">
							<c:forEach items="${lov_charge_type }" var="item">
								<c:if test="${activity.charge_type eq  item.key}">
									<option value="${item.key }" selected>${item.value}</option>
								</c:if>
								<c:if test="${activity.charge_type ne  item.key}">
					...
					<div class="appmsg_edit_item">
						...
						<select class="frm_input_box" style="width:100%"name="status" id="status">
							<c:forEach items="${lov_activity_status }" var="item">
								<c:if test="${activity.status eq  item.key}">
									<option value="${item.key }" selected>${item.value}</option>
								</c:if>
								<c:if test="${activity.status ne  item.key}">
									<option value="${item.key }">${item.value}</option>
								</c:if>
							</c:forEach>
						</select>
						...
						<select class="frm_input_box" style="width:100%"name="ispublish" id="ispublish">
							<c:forEach items="${lov_activity_ispublish}" var="item">
								<c:if test="${activity.ispublish eq  item.key}">
									<option value="${item.key }" selected>${item.value}</option>
								</c:if>
								<c:if test="${activity.ispublish ne  item.key}">
									<option value="${item.key }">${item.value}</option>
								</c:if>
							</c:forEach>
						</select>
						...
						<select class="frm_input_box" style="width:100%"name="islive" id="islive">
							<c:forEach items="${lov_activity_islive}" var="item">
								<c:if test="${activity.islive eq  item.key}">
									<option value="${item.key }" selected>${item.value}</option>
								</c:if>
								<c:if test="${activity.islive ne  item.key}">
									<option value="${item.key }">${item.value}</option>
								</c:if>
							</c:forEach>
						</select>
							...
							<div class="appmsg_edit_item live_type" style="display:none">
								...
								<select class="frm_input_box" style="width:100%"name="live_parameter" id="live_parameter">
									<c:forEach items="${lov_activity_live_parameter}" var="item">
										<c:if test="${activity.live_parameter eq  item.key}">
											<option value="${item.key }" selected>${item.value}</option>
										</c:if>
										<c:if test="${activity.live_parameter ne  item.key}">
											<option value="${item.key }">${item.value}</option>
										</c:if>
									</c:forEach>
								</select>
						...
						<div class="appmsg_edit_item">
						<label for="" class="frm_label">是否需要报名&nbsp;<span style="color:red;">*</span></label>
						<select class="frm_input_box" style="width:100%"name="isregist" id="isregist">
							<c:forEach items="${lov_activity_isregist}" var="item">
								<c:if test="${activity.isregist eq  item.key}">
									<option value="${item.key }" selected>${item.value}</option>
								</c:if>
								<c:if test="${activity.isregist ne  item.key}">
									<option value="${item.key }">${item.value}</option>
								</c:if>
							</c:forEach>
						</select>
					...
					<div class="appmsg_edit_item">
						...
						<select class="frm_input_box" style="width:100%"name="type" id="type">
							<c:forEach items="${lov_activity_type }" var="item">
								<c:if test="${activity.type eq  item.key}">
									<option value="${item.key }" selected>${item.value}</option>
								</c:if>
								<c:if test="${activity.type ne  item.key}">
									<option value="${item.key }">${item.value}</option>
								</c:if>
							</c:forEach>
						</select>
					...
					<div class="appmsg_edit_item">
						...
						<select class="frm_input_box" style="width:100%"name="display_member" id="display_member">
							<c:forEach items="${lov_activity_displaymenber }" var="item">
								<c:if test="${activity.display_member eq  item.key}">
									<option value="${item.key }" selected>${item.value}</option>
								</c:if>
								<c:if test="${activity.display_member ne  item.key}">
									<option value="${item.key }">${item.value}</option>
								</c:if>
							</c:forEach>
						</select>
					...
					<div class="appmsg_edit_item">
						<label for="" class="frm_label">人数限制</label>
						<span class="frm_input_box">
							<input type="text" name="limit_number"  value="${activity.limit_number}" class="frm_input js_author ">
						</span>
					</div>
				</form>
				...
				<div class="js_desc_area dn appmsg_edit_item" >
					...
					<span class="frm_textarea_box">
						<textarea rows="3" style="width:100%;border:0px;" name="desc">${activity.remark}</textarea> 
					</span>
				</div>
				<div class="appmsg_edit_item">
					...
					<p class="js_cover upload_preview">
						<img src="<%=path%>/mkattachment/download?fileName=${activity.logo}&flag=headImage"></img>
						<a class="js_removeCover" href="javascript:void(0);" style="vertical-align: bottom;margin-left: 10px;">删除</a>
	                </p>
				</div>
				<div class="appmsg_edit_item">
