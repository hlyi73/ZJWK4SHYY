...
<body>
	 <div id="operDiv" class="resource_menu zjwk_fg_nav_2">
		     <a href="javascript:void(0)" onclick='toConOwner()' style="padding:5px 8px;" class="con_owner">已关联</a>
								...
								<c:if test="${contact.firstname eq char1 }">
									...
									<li class="contact_list" contactid="${contact.rowid}" conname="${contact.conname}" conmobile="${contact.phonemobile}">
										<c:if test="${contact.type eq 'friend'}">
											<a href="<%=path%>/out/user/card?partyId=${contact.rowid}&flag=RM" onclick="return redirect('${contact.rowid}',this)" style="background-color:#fff;" target="_parent" chooseId='${ contact.rowid}'> 
										...
										<c:if test="${contact.type ne 'friend'}">
											<a href="<%=path%>/contact/modify?rowId=${contact.rowid}&orgId=${contact.orgId}" onclick="return redirect('${contact.rowid}',this)" style="background-color:#fff;" target="_parent" chooseId='${ contact.rowid}'>
												<div style="width:100%; background-color:#fff;">
													<div class="content" style="text-align: left">
														<div style="float:left;line-height:25px;">${contact.conname }</div>
														<div style="clear:both"></div>
