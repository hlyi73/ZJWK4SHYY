...
// 				$(this).parent().find(":hidden[name=vflag]").val("del");
// 			});
		}
...
<body style="background: #F5F5F5;">
	...
	<div id="task-create" class=" ">
		...
		<div class="wrapper">
			...
			<form name="tasvalForm" method="post" action="<%=path %>/tas/saveval">
			    ...
			    <c:if test="${fn:length(tasValueList) > 0}">
					<div class="form-group titleList">
						<label class="control-label" for="realname">请填写价值主张</label>
						
						<c:forEach var="v" items="${tasValueList}">
							<div class="sValeCon">
								<div style="width: 90%;float: left">
								    <!-- value id -->
								    <input type="hidden" name="vid" value="${v.id}" />
					...
					<div>
						<input type="btn" class="btn btn-block valSave" value="保    存" style="cursor:pointer;background: #55a1e3;width: 100%;text-align: align;">
					</div>
				</c:if>
				<c:if test="${fn:length(tasValueList) == 0}">
