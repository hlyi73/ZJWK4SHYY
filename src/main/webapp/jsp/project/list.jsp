...
<script type="text/javascript">
    ...
    function topage(){
		...
		$.ajax({
		      ...
		      async: false,
		      data: {currpage:currpage,viewtype:'${viewtype}',crmId:'${crmId}',pagecount:'10'} || {},
		      dataType: 'text',
		      success: function(data){
					...
					if(d != ""){
						...
						$(d).each(function(i){
							val+= '<a href="<%=path%>/project/detail?rowId='
...
<body>
	<div id="site-nav" class="navbar">
		...
		<h3 style="padding-right:45px;">项目列表</h3>
		<div class="act-secondary">
			<a
				href="<%=path%>/project/add?openId=${openId}&publicId=${publicId}" style="font-size:35px;font-weight:bold;color:#fff;padding:0px 10px 0px 10px;">+</a>
		</div>
	...
	<div class="site-recommend-list page-patch listContainer">
		<div class="list-group listview" id="div_oppty_list">
			<c:forEach items="${proList}" var="proj">
				<a href="<%=path%>/project/detail?rowId=${proj.rowid}&openId=${openId}&publicId=${publicId}"
					...
					<div class="list-group-item-bd">
						...
						<div class="content">
							<h1>${proj.name }&nbsp;<span
									style="color: #AAAAAA; font-size: 12px;">${proj.assigner}</span>
							</h1>
							<p>开始：${proj.startdate}</p>
							<p>结束：${proj.enddate}</p>
						</div>
					</div>
